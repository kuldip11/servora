...
describe("PriceRulesPanel coverage", () => {
  beforeEach(() => {
    ...
    h.getError.mockReturnValue("Save failed");
  });
  ...
  it("covers rendering rules, descriptions, removal and fully scoped save", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Rule fulfillment type"), {
      target: { value: "DELIVERY" },
    });
    fireEvent.click(screen.getByRole("checkbox"));
    fireEvent.change(screen.getByLabelText("Rule start date"), {
      target: { value: "2026-09-05" },
    });
    fireEvent.change(screen.getByLabelText("Rule end date"), {
      target: { value: "2026-09-06" },
    });
    fireEvent.change(screen.getByLabelText("Rule start time"), {
      target: { value: "10:00" },
    });
    fireEvent.change(screen.getByLabelText("Rule end time"), {
      target: { value: "11:00" },
    });
    fireEvent.change(screen.getByLabelText("Customer group scope"), {
      target: { value: "g1" },
    });
    fireEvent.change(screen.getByLabelText("Rule price"), {
      target: { value: "99.5" },
    });
    fireEvent.change(screen.getByLabelText("Rule priority"), {
      target: { value: "4" },
    });
