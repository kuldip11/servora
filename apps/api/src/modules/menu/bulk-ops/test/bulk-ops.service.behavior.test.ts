...
vi.mock("@/modules/menu/change-log/menu-change-log", () => ({
  menuChangeLog: { recordMany: m.recordMany },
}));
...
const auth = (): AuthContext => ({
  ...
  permissions: ["menu:update", "menu:delete"],
  roles: [],
  requestId: "r",
  ipAddress: "127.0.0.1",
});
...
describe("bulk ops service comprehensive coverage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    m.findItemScopes.mockResolvedValue([
      { id: "i1", branchId: "b1" },
      { id: "i2", branchId: null },
    ]);
  ...
  it("updates status with and without a reason and records changes", async () => {
    await expect(
      bulkOpsService.updateItemsStatus(auth(), ["i1", "i2"], "ACTIVE", "back"),
    ...
    expect(m.recordMany).toHaveBeenLastCalledWith(expect.anything(), [
      expect.objectContaining({
        entityId: "i1",
        changeType: "UPDATED",
        diff: { status: "ACTIVE", reason: "back" },
      }),
      expect.objectContaining({ entityId: "i2" }),
    ]);
    await bulkOpsService.updateItemsStatus(auth(), ["i1"], "HIDDEN");
    expect(m.recordMany).toHaveBeenLastCalledWith(expect.anything(), [
      expect.objectContaining({ diff: { status: "HIDDEN", reason: null } }),
    ]);
  });

  it("covers category, tags, modifiers, pricing and delete operations", async () => {
    await expect(
      bulkOpsService.updateItemsCategory(auth(), ["i1"], "c1"),
    ).resolves.toEqual({ updated: 2 });
    await expect(
      bulkOpsService.bulkSetItemTags(auth(), ["i1"], ["tag"], "add"),
    ).resolves.toEqual({ updated: 2 });
    await expect(
      bulkOpsService.bulkSetItemModifierGroups(
        auth(),
        ["i1"],
        ["m1"],
        "replace",
    ...
    await expect(
      bulkOpsService.bulkUpdatePrice(auth(), ["i1"], 10, "increase"),
    ).resolves.toMatchObject({ updated: 2 });
    await expect(
      bulkOpsService.bulkDeleteItems(auth(), ["i1"]),
    ).resolves.toEqual({ deleted: 2, protected: 0 });
    expect(m.requirePermission).toHaveBeenCalledWith(
      expect.anything(),
      "menu:delete",
    );
    expect(m.recordMany).toHaveBeenCalledWith(expect.anything(), [
      expect.objectContaining({ changeType: "DELETED", diff: {} }),
    ]);
  });

  it("propagates permission, scope and repository failures", async () => {
    m.requirePermission.mockImplementationOnce(() => {
      throw new Error("denied");
    });
    await expect(
      bulkOpsService.updateItemsCategory(auth(), ["i1"], "c1"),
    ).rejects.toThrow("denied");
    m.assertMenuResourceBranch.mockImplementationOnce(() => {
      throw new Error("scope");
    });
    await expect(
      bulkOpsService.bulkSetItemTags(auth(), ["i1"], [], "remove"),
    ).rejects.toThrow("scope");
    m.findItemScopes.mockResolvedValueOnce([]);
    m.bulkUpdatePrice.mockRejectedValueOnce(new Error("db"));
    await expect(
      bulkOpsService.bulkUpdatePrice(auth(), [], 1, "set"),
    ).rejects.toThrow("db");
  });
});
