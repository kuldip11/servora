...
vi.mock("@/features/tables/components/TableFormModal", () => ({ TableFormModal: ({ open, mode, onSubmit, onClose }: any) => open ? <div role="dialog"><h2>{mode}-table</h2><button onClick={() => onSubmit({ name: "T9", capacity: "4", section: "Hall", branchId: "b1" })}>submit-{mode}</button>{mode === "add" ? <button onClick={() => onSubmit({ name: "T0", capacity: "2", section: "", branchId: "" })}>submit-add-no-branch</button> : null}<button onClick={onClose}>close-{mode}</button></div> : null }));
vi.mock("@pos/ui", () => ({
...
describe("TablesPage coverage", () => {
  ...
  it("loads takeaway QR and transfers/merges occupied tables", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Transfer" })); expect(screen.getByRole("heading", { name: "Transfer A1" })).toBeTruthy(); const selects = screen.getAllByRole("combobox"); fireEvent.change(selects.at(-1)!, { target: { value: "t2" } }); fireEvent.change(screen.getByLabelText("Reason (optional)"), { target: { value: "guest move" } }); fireEvent.click(screen.getAllByRole("button", { name: "Transfer" }).at(-1)!); expect(mocks.transfer).toHaveBeenCalledWith(expect.objectContaining({ orderId: "o1", newTableId: "t2", reason: "guest move" }), expect.any(Object)); const transferOptions = mocks.transfer.mock.calls.at(-1)?.[1]; act(() => transferOptions.onSuccess());
  ...
  it("covers close, error, permission, aggregate mutation, disabled QR and pagination paths", async () => {
    ...
    fireEvent.click(screen.getAllByLabelText("Edit table")[1]!);
    fireEvent.click(screen.getByRole("button", { name: "close-edit" }));
    fireEvent.change(screen.getByLabelText("Search tables"), { target: { value: "missing" } });
    ...
    await waitFor(() => expect(consoleError).toHaveBeenCalledWith("Unable to load takeaway QR", expect.any(Error)));
    mocks.getTakeaway.mockResolvedValueOnce({ branchId: "b1", branchName: "Central", enabled: false, token: "off" });
    fireEvent.click(screen.getByRole("button", { name: /Takeaway QR/ }));
    expect(await screen.findByText(/Takeaway ordering is disabled/)).toBeTruthy();
    mocks.regenTakeaway.mockRejectedValueOnce(new Error("regen"));
    fireEvent.click(screen.getByRole("button", { name: "Regenerate" }));
    await waitFor(() => expect(consoleError).toHaveBeenCalledWith("Unable to regenerate takeaway QR", expect.any(Error)));
    ...
    fireEvent.click(screen.getByRole("button", { name: /Add Table/ }));
    fireEvent.click(screen.getByRole("button", { name: "submit-add-no-branch" }));
    expect(mocks.add).not.toHaveBeenCalledWith(expect.objectContaining({ name: "T0" }), expect.anything());
    fireEvent.click(screen.getByRole("button", { name: "close-add" }));
    fireEvent.click(screen.getAllByLabelText("Remove table")[0]!);
    expect(mocks.del).toHaveBeenCalled();
    fireEvent.change(screen.getAllByLabelText("status-AVAILABLE")[0]!, { target: { value: "CLEANING" } });
    expect(mocks.status).toHaveBeenCalled();
    unmount();

    mocks.has.mockImplementation((permission: string) => permission !== "orders:update" && permission !== "tables:create");
  ...
  it("covers mutation success callbacks, missing transfer order, merge errors and large-table observer", async () => {
    ...
    const addOptions = mocks.add.mock.calls.at(-1)?.[1]; act(() => addOptions.onSuccess());
    await waitFor(() => expect(screen.queryByRole("heading", { name: "add-table" })).toBeNull());
    fireEvent.click(screen.getAllByLabelText("Edit table")[1]!);
    fireEvent.click(screen.getByRole("button", { name: "submit-edit" }));
    const updateOptions = mocks.update.mock.calls.at(-1)?.[1]; act(() => updateOptions.onSuccess());

    fireEvent.click(screen.getAllByLabelText("Show table QR code")[1]!);
    fireEvent.click(screen.getByRole("button", { name: "Regenerate" }));
    const regenOptions = mocks.regen.mock.calls.at(-1)?.[1]; act(() => regenOptions.onSuccess({ ...t2, publicQrToken: "new-q" }));
    expect(screen.getAllByText(/new-q/).length).toBeGreaterThan(0);
    fireEvent.click(screen.getByRole("button", { name: "modal-x" }));
    ...
    fireEvent.click(screen.getAllByRole("button", { name: "Transfer" }).at(-1)!);
    expect(mocks.transfer).not.toHaveBeenCalled();
    fireEvent.click(screen.getAllByRole("button", { name: "modal-x" }).at(-1)!);
    unmount();

    mocks.orders.current = [{ id: "o1", tableId: "t1" }, { id: "o2", tableId: "t2", mergedIntoOrderId: "x" }, { id: "order-long-12345678", tableId: "missing" }];
    mocks.merge.mockRejectedValueOnce(new Error("merge failed"));
    render(<TablesPage />);
    fireEvent.click(screen.getByRole("button", { name: "Merge" }));
    fireEvent.change(screen.getAllByRole("combobox").at(-1)!, { target: { value: "order-long-12345678" } });
    fireEvent.click(screen.getByRole("button", { name: "Merge tables" }));
    await waitFor(() => expect(mocks.error).toHaveBeenCalledWith(expect.any(Error), "Unable to merge tables"));
    fireEvent.click(screen.getByRole("button", { name: "Cancel" }));

    let observerCallback: ((entries: Array<{ isIntersecting: boolean }>) => void) | undefined;
    const disconnect = vi.fn();
    vi.stubGlobal("IntersectionObserver", class { constructor(cb: any) { observerCallback = cb; } observe() {} disconnect() { disconnect(); } });
    const many = Array.from({ length: 40 }, (_, i) => ({ ...t2, id: `m${i}`, name: `M${i}` }));
    ...
    await waitFor(() => expect(screen.queryByText("Loading more tables…")).toBeNull());
    view.unmount();
  });
