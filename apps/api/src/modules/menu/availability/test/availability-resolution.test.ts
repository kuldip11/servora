...
describe("resolveEffectiveAvailability additional branches", () => {
  it("covers branch/channel/base/recipe causes and presentation fallbacks", () => {
    expect(
      resolveEffectiveAvailability(
        evidence({ channelOverride: { status: "OUT_OF_STOCK" } }),
      ).availabilityCause,
    ).toBe("CHANNEL_OVERRIDE");
    expect(
      resolveEffectiveAvailability(
        evidence({
          branchOverride: {
            ...
            taxRate: "7.00",
            isHidden: true,
            availabilityReason: "branch",
          },
        }),
      ),
    ).toMatchObject({
      availabilityCause: "BRANCH_OVERRIDE",
      effectiveTaxRate: "7.00",
      isHidden: true,
      availabilityReason: "branch",
    });
    expect(
      resolveEffectiveAvailability(
        evidence({ branchOverride: { status: null, isHidden: true } }),
      ).availabilityCause,
  ...
  it("uses manual override default reason and recognizes all schedule reason forms", () => {
    ...
    ).toBe("Manual availability override");
    for (const reason of [
      ...
      expect(
        resolveEffectiveAvailability(
          evidence({ resolvedStatus: { status: "OUT_OF_STOCK", reason } }),
        ).availabilityCause,
      ).toBe("SCHEDULE");
    }
  });
