...
const mocks = vi.hoisted(() => ({
  ...
  impact: { current: { data: undefined, isLoading: false } as any },
  add: vi.fn(),
  update: vi.fn(),
  waste: vi.fn(),
  createReason: vi.fn(),
...
vi.mock("@/features/inventory/hooks/useInventoryTransactions", () => ({
  useInventoryTransactions: () => ({ data: mocks.transactions.current }),
}));
vi.mock("@/features/inventory/hooks/useWasteReasons", () => ({
  useWasteReasons: () => ({ data: mocks.reasons.current }),
}));
vi.mock("@/features/inventory/hooks/useInventoryRecipeImpact", () => ({
  useInventoryRecipeImpact: () => mocks.impact.current,
}));
vi.mock("@/features/inventory/hooks/useLogInventoryWaste", () => ({
  useLogInventoryWaste: () => ({ mutate: mocks.waste, isPending: false }),
  useCreateWasteReason: () => ({
    mutate: mocks.createReason,
    isPending: false,
  }),
}));
...
vi.mock("@pos/ui", () => ({
  ...
  IconButton: ({ "aria-label": label }: any) => (
...
describe("InventoryPage coverage", () => {
  beforeEach(() => {
    ...
    mocks.reasons.current = [{ id: "wr1", label: "Spoilage" }];
    mocks.impact.current = {
      data: {
        impacts: [
          {
            kind: "ITEM",
  ...
  it("renders inventory, filters, transactions and stock updates", async () => {
    ...
    expect(
      screen.getByText("52 items tracked in the current view"),
    ).toBeTruthy();
    expect(screen.getByText("Low Stock Alerts (1)")).toBeTruthy();
    expect(screen.getByText("Chicken — 2 KG")).toBeTruthy();
    ...
    fireEvent.change(screen.getByLabelText("Filter inventory by stock level"), {
      target: { value: "low" },
    });
    ...
    expect(
      screen.getByRole("heading", { name: "Update Stock: Chicken" }),
    ).toBeTruthy();
  ...
  it("adds items and handles waste and recipe impact flows", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Waste reason"), {
      target: { value: "wr1" },
    });
    fireEvent.change(screen.getByLabelText("Notes (optional)"), {
      target: { value: "trim" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Log Waste" }));
    await waitFor(() =>
      expect(mocks.waste).toHaveBeenCalledWith(
    ...
    const wasteOptions = mocks.waste.mock.calls.at(-1)?.[1];
    act(() => wasteOptions.onSuccess());
    fireEvent.click(screen.getAllByRole("button", { name: "Log waste" })[0]!);
    fireEvent.change(screen.getByLabelText("New waste reason"), {
      target: { value: "Prep trim" },
    });
    mocks.createReason.mockImplementation((_name, opts) =>
      opts.onSuccess({ id: "wr2" }),
    );
    fireEvent.click(screen.getByRole("button", { name: "Add" }));
    expect(mocks.createReason).toHaveBeenCalledWith(
      "Prep trim",
      expect.any(Object),
    );
    fireEvent.click(
    ...
    expect(
      screen.getByRole("heading", { name: "Recipe impact: Chicken" }),
    ).toBeTruthy();
    expect(screen.getByText("Base item")).toBeTruthy();
  ...
  it("covers aggregate grouping and empty states", () => {
    ...
    mocks.items.current = {
      data: { items: [item, item2], pagination: { total: 2 } },
      isLoading: false,
    };
    mocks.low.current = [];
    mocks.transactions.current = [];
    ...
    act(() => useAuthStore.setState({ branchId: "b1" }));
    rerender(<InventoryPage />);
    expect(screen.getByText("No inventory items")).toBeTruthy();
  });
  it("covers success resets, clear-search, neutral transactions and recipe impact empty/loading states", async () => {
    mocks.transactions.current = [
      {
        id: "t3",
        transactionType: "OUT",
    ...
    mocks.impact.current = { data: undefined, isLoading: true };
    const loadingView = render(<InventoryPage />);
    fireEvent.click(
      screen.getAllByRole("button", { name: "Recipe impact" })[0]!,
    );
    expect(screen.getByText("Loading recipe impact…")).toBeTruthy();
    loadingView.unmount();

    mocks.impact.current = { data: { impacts: [] }, isLoading: false };
    render(<InventoryPage />);
    fireEvent.click(
      screen.getAllByRole("button", { name: "Recipe impact" })[0]!,
    );
    expect(
      screen.getByText(/not currently a required auto-deduction input/),
    ).toBeTruthy();
  });

  it("covers aggregate page-size reset and missing branches list", () => {
    useAuthStore.setState({ branchId: "all" });
    mocks.branches.current = undefined as any;
    mocks.items.current = {
      data: { items: [item, item2], pagination: { total: 60 } },
      isLoading: false,
    };
    render(<InventoryPage />);
    fireEvent.click(screen.getByRole("button", { name: "size" }));
