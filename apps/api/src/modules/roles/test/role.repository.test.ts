...
vi.mock("@/db", () => {
  ...
  m.selectFrom.mockImplementation(() => ({ where: m.selectWhere }));
  return {
    db: {
      query: { roles: { findMany: m.findMany, findFirst: m.findFirst } },
      insert: m.insert,
      update: m.update,
      select: m.select,
    },
...
describe("role repository coverage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    m.findMany.mockImplementation(async (o?: any) => {
      o?.orderBy?.(
    ...
    m.insertReturning.mockResolvedValue([{ id: "r1" }]);
    m.updateReturning.mockResolvedValue([{ id: "r1" }]);
    m.selectWhere.mockResolvedValue([{ count: 2 }]);
  });
  it("covers list/find/create", async () => {
    await expect(roleRepository.listForTenant("t1")).resolves.toEqual([
      { id: "r1" },
    ]);
    await expect(roleRepository.findTenantRole("t1", "r1")).resolves.toEqual({
      id: "r1",
    });
    await expect(
      roleRepository.findByNameAndScope("t1", " Role ", "TENANT"),
    ).resolves.toEqual({ id: "r1" });
    await expect(
      roleRepository.create("t1", {
        name: " Role ",
        description: "  desc ",
        scope: "TENANT",
      }),
    ).resolves.toEqual({ id: "r1" });
    await roleRepository.create("t1", {
      name: "Role",
      description: "   ",
      scope: "BRANCH",
    });
  ...
  it("covers update value branches", async () => {
    await expect(
      roleRepository.update("r1", { name: " X ", description: " desc " }),
    ).resolves.toEqual({ id: "r1" });
