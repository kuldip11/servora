...
vi.mock("@/features/branches/hooks/useBranches", () => ({
  useBranches: mocks.branches,
}));
vi.mock("@/features/branches/hooks/useCreateBranch", () => ({
  useCreateBranch: () => ({ mutate: mocks.add, isPending: false }),
}));
vi.mock("@/features/branches/hooks/useUpdateBranch", () => ({
  useUpdateBranch: () => ({ mutate: mocks.update, isPending: false }),
}));
vi.mock("@/features/branches/hooks/useDeactivateBranch", () => ({
  useDeactivateBranch: () => ({ mutate: mocks.deactivate }),
}));
...
vi.mock("@/features/branches/components/BranchCard", () => ({
  BranchCard: ({ branch, onEdit, onDeactivate }: any) => (
    <div>
      <span>{branch.name}</span>
      <button onClick={() => onEdit(branch)}>edit-{branch.id}</button>
...
vi.mock("@/features/branches/components/BranchFormModal", () => ({
  BranchFormModal: ({ mode, open, onClose, onSubmit }: any) =>
    open ? (
...
describe("BranchesPage coverage", () => {
  beforeEach(() => {
    ...
    mocks.update.mockImplementation((_values, options) =>
      options?.onSuccess?.(),
    );
    vi.stubGlobal(
      "confirm",
  ...
  it("renders loading and permission-aware empty states", () => {
    ...
    expect(screen.getAllByRole("button", { name: /Add Branch/ })).toHaveLength(
      1,
    );
  });
  ...
  it("adds, edits and deactivates branches", async () => {
    ...
    expect(
      screen.getByText(
        "1 branch — each has its own menu, tables, staff, and orders",
      ),
    ).toBeTruthy();
