...
vi.mock("../staff.controller", () => ({ staffController: m }));
import { staffRouter } from "../staff.route";
const r = (method: string, path: string) =>
  (staffRouter as any).routes.find(
    (x: any) => x.method === method && x.path === path,
  );
describe("staff route callbacks", () => {
  beforeEach(() => {
    for (const f of Object.values(m)) f.mockResolvedValue("ok");
  });
  it("executes every handler", async () => {
    const auth = {};
    await r("GET", "/api/staff/").handler({ auth, query: { page: 1 } });
    const set: any = {};
    await r("POST", "/api/staff/").handler({ auth, body: {}, set });
    expect(set.status).toBe(201);
    await r("PATCH", "/api/staff/:id").handler({
      auth,
      params: { id: "u1" },
      body: {},
    });
    await r("DELETE", "/api/staff/:id").handler({ auth, params: { id: "u1" } });
    expect((staffRouter as any).routes).toHaveLength(4);
  });
});
