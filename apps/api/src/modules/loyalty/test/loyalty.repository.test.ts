...
const m = vi.hoisted(() => {
  ...
  const del = vi.fn();
  return {
    tenantFindFirst,tierFindMany,tierFindFirst,customerFindMany,customerFindFirst,
    insertReturning,insertValues,updateReturning,updateWhereResolved,updateWhere,updateSet,deleteWhere,
    selectLimit,selectWhere,selectInnerJoin,selectFrom,select,insert,update,del,
  };
});
  ...
  return {
    db: {
      query: {
        ...
        customerLoyaltyTiers: { findMany: m.tierFindMany, findFirst: m.tierFindFirst },
        customers: { findMany: m.customerFindMany, findFirst: m.customerFindFirst },
      },
      insert: m.insert,
      update: m.update,
      delete: m.del,
      select: m.select,
      __setSelectedRows: (rows: unknown[]) => { selectedRows = rows; },
    },
  };
});
...
const setSelectedRows = (rows: unknown[]) => (db as unknown as { __setSelectedRows(rows: unknown[]): void }).__setSelectedRows(rows);
const tier = { id:"tier-1", tenantId:"t1", organizationId:null, name:"Gold", discountPercent:"10.00", discountFixed:null } as any;
const customer = { id:"c1", tenantId:"t1", name:"A", email:null, phone:"111", loyaltyTierId:null, organizationCustomerId:null, loyaltyTier:null } as any;
...
describe("loyaltyRepository comprehensive coverage", () => {
  ...
  it("lists and finds applicable tiers with and without organization scope", async () => {
    await expect(loyaltyRepository.listApplicableTiers("t1")).resolves.toEqual([tier]);
    await expect(loyaltyRepository.findApplicableTier("t1","tier-1")).resolves.toBe(tier);
  ...
  it("covers tenant tier CRUD and customer reads", async () => {
    ...
    expect(await loyaltyRepository.createTier({ tenantId:"t1", name:"Gold", discountPercent:"10.00" } as any)).toBe(tier);
    expect(await loyaltyRepository.updateTier("t1","tier-1",{ name:"G" } as any)).toBe(tier);
  ...
  it("finds organization tier for a customer and handles every early exit", async () => {
    ...
    await expect(loyaltyRepository.findOrganizationTierForCustomer("t1","missing")).resolves.toBeUndefined();

    m.customerFindFirst.mockResolvedValueOnce(customer);
    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:null });
    await expect(loyaltyRepository.findOrganizationTierForCustomer("t1","c1")).resolves.toBeUndefined();
    ...
    m.customerFindFirst.mockResolvedValueOnce({ ...customer, organizationCustomerId:"identity", phone:null });
    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:"org-1" });
    setSelectedRows([{ id:"c1" },{ id:"c2" }]);
    m.customerFindMany.mockResolvedValueOnce([{ ...customer, loyaltyTier:null },{ ...customer, id:"c2", loyaltyTier:orgTier }]);
    await expect(loyaltyRepository.findOrganizationTierForCustomer("t1","c1")).resolves.toEqual(orgTier);

    m.customerFindFirst.mockResolvedValueOnce({ ...customer, organizationCustomerId:null, phone:null });
    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:"org-1" });
    setSelectedRows([{ id:"c1" }]);
    m.customerFindMany.mockResolvedValueOnce([{ ...customer, loyaltyTier:null }]);
    await expect(loyaltyRepository.findOrganizationTierForCustomer("t1","c1")).resolves.toBeUndefined();
  });

  it("covers organization tier CRUD", async () => {
    const orgTier = { ...tier, organizationId:"org-1", tenantId:null };
    m.tierFindMany.mockImplementationOnce(async (options?: any) => { options?.orderBy?.({ name: "name" }, { asc: (value: unknown) => value }); return [orgTier]; });
    ...
    expect(await loyaltyRepository.listOrganizationTiers("org-1")).toEqual([orgTier]);
    expect(await loyaltyRepository.createOrganizationTier("org-1",{ name:"Org", discountPercent:"5.00" } as any)).toEqual(orgTier);
    expect(await loyaltyRepository.findOrganizationTier("org-1","tier-1")).toEqual(orgTier);
    expect(await loyaltyRepository.updateOrganizationTier("org-1","tier-1",{ name:"Updated" } as any)).toEqual(orgTier);
  ...
  it("returns local phone matches and short-circuits an empty phone", async () => {
    ...
    await expect(loyaltyRepository.findCustomersByPhone("t1","111")).resolves.toEqual([customer]);
    m.customerFindMany.mockResolvedValueOnce([]);
    await expect(loyaltyRepository.findCustomersByPhone("t1","")).resolves.toEqual([]);
  ...
  it("handles missing organization and ambiguous/no sibling phone matches", async () => {
    ...
    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:null });
    await expect(loyaltyRepository.findCustomersByPhone("t1","111")).resolves.toEqual([]);
  ...
  it("materializes sibling identity, preserves existing identity, and handles concurrent creation", async () => {
    ...
    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:"org-1" });
    m.selectLimit.mockResolvedValueOnce([{ id:"s1", tenantId:"t2", name:"Sibling", email:"s@x", phone:"111", organizationCustomerId:null }]);
    m.insertReturning.mockResolvedValueOnce([{ ...customer, id:"created" }]);
    m.customerFindFirst.mockResolvedValueOnce({ ...customer, id:"created" });
    await expect(loyaltyRepository.findCustomersByPhone("t1","111")).resolves.toEqual([{ ...customer, id:"created" }]);
    expect(m.updateSet).toHaveBeenCalledWith(expect.objectContaining({ organizationCustomerId:"s1" }));

    m.customerFindMany.mockResolvedValueOnce([]).mockResolvedValueOnce([{ ...customer, id:"concurrent" }]);
    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:"org-1" });
    m.selectLimit.mockResolvedValueOnce([{ id:"s1", tenantId:"t2", name:"Sibling", email:null, phone:"111", organizationCustomerId:"identity" }]);
    await expect(loyaltyRepository.findCustomersByPhone("t1","111")).resolves.toEqual([{ ...customer, id:"concurrent" }]);
  });
  ...
  it("finds or seeds organization customer identities", async () => {
    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:null });
    await expect(loyaltyRepository.findOrganizationCustomerIdentity("t1","111")).resolves.toBeNull();
    ...
    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:"org-1" });
    m.selectLimit.mockResolvedValueOnce([{ id:"s1", tenantId:"t2", organizationCustomerId:null }]);
    await expect(loyaltyRepository.findOrganizationCustomerIdentity("t1","111")).resolves.toBe("s1");
    expect(m.updateSet).toHaveBeenLastCalledWith(expect.objectContaining({ organizationCustomerId:"s1" }));

    m.tenantFindFirst.mockResolvedValueOnce({ organizationId:"org-1" });
    m.selectLimit.mockResolvedValueOnce([{ id:"s1", tenantId:"t2", organizationCustomerId:"identity" }]);
    await expect(loyaltyRepository.findOrganizationCustomerIdentity("t1","111")).resolves.toBe("identity");
  });

  it("covers customer identity setter and customer create/update", async () => {
    m.updateReturning.mockResolvedValueOnce([customer]);
    expect(await loyaltyRepository.setOrganizationCustomerIdentity("t1","c1","identity")).toBe(customer);
    m.insertReturning.mockResolvedValueOnce([customer]);
    expect(await loyaltyRepository.createCustomer({ tenantId:"t1", name:"A" } as any)).toBe(customer);
    m.updateReturning.mockResolvedValueOnce([customer]);
    expect(await loyaltyRepository.updateCustomer("t1","c1",{ name:"B" } as any)).toBe(customer);
  });
});
