...
describe("InventoryPage coverage", () => {
  ...
  it("renders inventory, filters, transactions and stock updates", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Quantity"), { target: { value: "4" } }); fireEvent.click(screen.getByRole("button", { name: "Update" })); await waitFor(() => expect(mocks.update).toHaveBeenCalled()); const updateOptions = mocks.update.mock.calls.at(-1)?.[1]; act(() => updateOptions.onSuccess());
  });
  it("adds items and handles waste and recipe impact flows", async () => {
    render(<InventoryPage />); fireEvent.click(screen.getAllByRole("button", { name: /Add Item/ })[0]!); fireEvent.change(screen.getByLabelText("Item name"), { target: { value: "Oil" } }); fireEvent.click(screen.getAllByRole("button", { name: "Add Item" }).at(-1)!); await waitFor(() => expect(mocks.add).toHaveBeenCalled()); const addOptions = mocks.add.mock.calls.at(-1)?.[1]; act(() => addOptions.onSuccess());
  ...
  it("covers success resets, clear-search, neutral transactions and recipe impact empty/loading states", async () => {
    mocks.transactions.current = [{ id: "t3", transactionType: "OUT", quantity: "2", createdAt: "2026-09-01T12:00:00Z", inventoryItem: { name: "Rice", unit: "KG" }, notes: "usage" }];
    const view = render(<InventoryPage />);
    expect(screen.getByText("OUT")).toBeTruthy();
    fireEvent.change(screen.getByLabelText("Search inventory"), { target: { value: "rice" } });
    ...
    fireEvent.click(screen.getAllByRole("button", { name: "Recipe impact" })[0]!);
    ...
    render(<InventoryPage />);
    fireEvent.click(screen.getAllByRole("button", { name: "Recipe impact" })[0]!);
    expect(screen.getByText(/not currently a required auto-deduction input/)).toBeTruthy();
  });
  ...
  it("covers aggregate page-size reset and missing branches list", () => {
    ...
    mocks.items.current = { data: { items: [item, item2], pagination: { total: 60 } }, isLoading: false };
    render(<InventoryPage />);
    fireEvent.click(screen.getByRole("button", { name: "size" }));
    fireEvent.click(screen.getByRole("button", { name: /Add Item/ }));
    expect(screen.getByRole("heading", { name: "Add Inventory Item" })).toBeTruthy();
  });
