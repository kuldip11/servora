...
describe("VariantAvailabilityPanel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    h.queryData.clear();
    for (const fn of [
  ...
  it("updates availability and finite stock counts", async () => {
    render(<VariantAvailabilityPanel itemId="i1" variants={variants} />);
    ...
    fireEvent.click(
      screen.getAllByRole("button", { name: "Stop tracking" })[0]!,
    );
    await waitFor(() =>
      expect(h.updateVariantAvailability).toHaveBeenCalledTimes(2),
    );
