...
vi.mock("@pos/ui", () => ({
  ...
  SelectMenu: ({
    ...
    <label>
      ...
      >
        {!options.some((o: any) => o.value === "") && <option value="" />}{" "}
        {options.map((o: any) => (
...
const props = (overrides: any = {}) => ({
  ...
  customerResults: [
    { id: "c1", name: "Ada", phone: "999" },
    { id: "c2", name: "No Contact" },
  ],
  ...
  perCoverRules: [
    { id: "r1", coverTier: "ADULT", price: 100 },
    { id: "r2", coverTier: null, price: null },
  ],
  perCoverPriceRuleId: "",
...
describe("OrderOptionsPanel interactions", () => {
  ...
  it("covers customer advanced controls and per-cover billing", () => {
    ...
    fireEvent.change(screen.getByLabelText("Customer group"), {
      target: { value: "g1" },
    });
    fireEvent.change(screen.getByLabelText("Billing mode"), {
      target: { value: "PER_COVER" },
    });
  ...
  it("covers no order types, empty tables and unavailable-table messaging", () => {
    const { rerender } = render(
      <OrderOptionsPanel {...props({ availableOrderTypes: [], tables: [] })} />,
    );
