...
describe("DifferentiatorsPage coverage", () => {
  ...
  it("explains orders and creates combo and promotion drafts with authoritative previews", async () => {
    ...
    itemSelects.forEach((select) => fireEvent.change(select, { target: { value: "i1" } }));
  ...
  it("covers load failures, empty states, engineering sort fallbacks, and explain failure", async () => {
    ...
    render(<DifferentiatorsPage />);
    expect(await screen.findByText("Everything is available in the selected scope.")).toBeTruthy();
    await waitFor(() => expect(mocks.toast).toHaveBeenCalledWith({ title: "Request failed", tone: "danger" }));

    mocks.menuEngineering.mockResolvedValueOnce([
      { menuItemId: "i3", menuItemName: "Alpha", variantName: "B", margin: 10, salesVolume: 2, quadrant: "STAR", recommendation: "A" },
      { menuItemId: "i4", menuItemName: "Alpha", variantName: "A", margin: 20, salesVolume: 5, quadrant: "STAR", recommendation: "B" },
    ]);
    fireEvent.click(screen.getByRole("button", { name: "Menu engineering" }));
    fireEvent.change(screen.getByLabelText("Analysis window"), { target: { value: "60" } });
    await waitFor(() => expect(mocks.menuEngineering).toHaveBeenCalledWith(60));
    fireEvent.change(screen.getByLabelText("Sort by"), { target: { value: "volume" } });
    fireEvent.change(screen.getByLabelText("Quadrant"), { target: { value: "ALL" } });
    fireEvent.change(screen.getByLabelText("Sort by"), { target: { value: "name" } });

    mocks.explain.mockRejectedValueOnce(new Error("explain"));
    fireEvent.click(screen.getByRole("button", { name: "Order explain" }));
    fireEvent.change(screen.getByPlaceholderText("Order UUID"), { target: { value: " order-2 " } });
    fireEvent.click(screen.getByRole("button", { name: "Explain" }));
  ...
  it("covers combo editing, percent pricing, add/remove slots, upcharges, and preview/create failures", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("Lunch combo"), { target: { value: "Percent Combo" } });
  ...
  it("covers promotion control variants, preview/create failures, and approval COMP/failure paths", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Guided builder" }));
    fireEvent.click(screen.getAllByRole("button", { name: "Create promotion" })[0]!);
    fireEvent.change(screen.getByPlaceholderText("Weekday special"), { target: { value: "Fixed Deal" } });
    fireEvent.change(screen.getByLabelText("Discount type"), { target: { value: "FIXED_AMOUNT" } });
    fireEvent.change(screen.getByLabelText("Amount off"), { target: { value: "20" } });
    fireEvent.change(screen.getByLabelText("Preview against menu item"), { target: { value: "i1" } });
    fireEvent.change(screen.getByPlaceholderText("LUNCH10"), { target: { value: "" } });

    mocks.previewPromotion.mockRejectedValueOnce(new Error("preview-promo"));
    fireEvent.click(screen.getByRole("button", { name: "Preview authoritative discount" }));
    await waitFor(() => expect(mocks.toast).toHaveBeenCalledWith({ title: "Request failed", tone: "danger" }));

    mocks.previewPromotion.mockResolvedValueOnce({ subtotal: 100, discountAmount: 20, totalAmount: 80 });
    fireEvent.click(screen.getByRole("button", { name: "Preview authoritative discount" }));
    ...
    fireEvent.click(screen.getByRole("button", { name: "Approval rules" }));
    fireEvent.change(screen.getByLabelText("Action"), { target: { value: "COMP" } });
    fireEvent.change(screen.getByLabelText("Threshold amount"), { target: { value: "100" } });
    fireEvent.change(screen.getByLabelText("Required role"), { target: { value: " Manager " } });
    fireEvent.click(screen.getByRole("button", { name: "Save threshold" }));
    await waitFor(() => expect(mocks.setThreshold).toHaveBeenCalledWith("COMP", { thresholdAmount: 100, requiresRole: "Manager" }));
    expect(mocks.toast).toHaveBeenCalledWith(expect.objectContaining({ title: "Comp threshold saved" }));
