...
const h = vi.hoisted(() => ({
  ...
  orders: {
    ...
    listAllCancellationReasons: vi.fn(),
...
describe("web service coverage", () => {
  beforeEach(() => {
    ...
    h.createTenant.mockResolvedValue({});
    h.updateTenant.mockResolvedValue({});
    h.archiveTenant.mockResolvedValue({});
  });
  ...
  it("covers business CRUD and normalization", async () => {
    ...
    await businessService.updateBranch("b1", {
      ...branch,
      status: "INACTIVE",
      customerQrEnabled: false,
    });
    await businessService.archiveBranch("b1");
    expect(h.org.create).toHaveBeenCalledWith(
      expect.objectContaining({ legalName: null, taxId: null }),
    );
    expect(h.createTenant).toHaveBeenCalledWith(
      expect.objectContaining({ organizationId: "o1", slug: null }),
    );
    expect(h.branches.create).toHaveBeenCalledWith(
    ...
    expect(h.branches.update).toHaveBeenCalledWith(
      "b1",
      expect.objectContaining({ onlineEnabled: false, isActive: false }),
    );
  });
  ...
  it("covers audit and cancellation-reason services", async () => {
    ...
    expect(h.audit.list).toHaveBeenNthCalledWith(1, {
      limit: 50,
      before: undefined,
    });
    expect(h.audit.list).toHaveBeenNthCalledWith(2, {
      limit: 10,
      before: "cursor",
    });
    ...
    await cancellationReasonsService.update("r1", {
      label: "Changed",
      isActive: false,
    });
  ...
  it("covers billing optional fields and split helpers", async () => {
    ...
    expect(h.billing.collectPayment).toHaveBeenNthCalledWith(1, {
      orderId: "o1",
      method: "CASH",
      amount: 100,
    });
    expect(h.billing.collectPayment).toHaveBeenNthCalledWith(2, {
      orderId: "o1",
      method: "CARD",
      amount: 50,
      billId: "b1",
