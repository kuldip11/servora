...
const makeProps = (
  overrides: Partial<MenuItemsContentProps> = {},
...
describe("MenuItemsContent coverage", () => {
  ...
  it("renders loading and empty category states", () => {
    const { rerender } = render(
      <MenuItemsContent {...makeProps({ isLoading: true })} />,
    );
    expect(screen.getByText("Loading spinner")).toBeTruthy();
    rerender(
      <MenuItemsContent {...makeProps({ categories: [], isLoading: false })} />,
    );
  ...
  it("filters, expands, edits and exercises item action controls", () => {
    ...
    const duplicate = vi.fn(
      (_: string, opts?: { onSuccess?: (item: MenuItem) => void }) =>
    ...
    render(
      <MenuItemsContent
        {...makeProps({
          ...
          publishFilter: "DRAFT",
          categories: [
            { id: "cat-1", name: "Drinks", menuItems: [itemWithVariant] },
          ] as never,
          setItemSearch,
          setFoodTypeFilter,
          setStatusFilter,
    ...
    expect(setItemForm).toHaveBeenCalledWith({
      categoryId: "cat-1",
      item: null,
    });
    ...
    expect(setItemForm).toHaveBeenLastCalledWith({
      categoryId: "cat-1",
  ...
  it("covers selection mode, existing override clearing and infinite scrolling", () => {
    ...
    const { rerender } = render(
      <MenuItemsContent
        {...makeProps({
          categories: [
            { id: "cat-1", name: "Drinks", menuItems: items },
          ] as never,
          toggleAvailMutation: { mutate: toggle },
        })}
      />,
    ...
    fireEvent.click(
      screen.getByLabelText("Clear manual availability override"),
    );
    ...
    const updater = selectedSetter.mock.calls.at(-1)?.[0] as (
      current: string[],
    ) => string[];
    expect(updater(["item-1", "item-2"])).toEqual(["item-2"]);
    fireEvent.keyDown(screen.getByRole("button", { name: "Tea 2" }), {
      key: "Enter",
    });
    const addUpdater = selectedSetter.mock.calls.at(-1)?.[0] as (
      current: string[],
    ) => string[];
