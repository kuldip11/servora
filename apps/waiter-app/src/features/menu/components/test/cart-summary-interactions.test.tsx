...
const combos: any[] = [
  {
    ...
    quantity: 1,
    courseNumber: 3,
    selections: [{ slotId: "slot1", optionIds: ["m2"] }],
  },
];
const menuById = new Map([
  ["m2", { id: "m2", name: "Fries", basePrice: 50 } as any],
]);
...
const makeProps = () => ({
  ...
  courseSequencingAvailable: true,
  courseMode: true,
...
describe("CartSummary interactions", () => {
  it("covers course sections, combos, edits, quantity and footer changes", () => {
    ...
    fireEvent.change(screen.getByLabelText("Order notes…"), {
      target: { value: "new" },
    });
    ...
    fireEvent.change(screen.getByLabelText("Course for Burger"), {
      target: { value: "3" },
    });
    fireEvent.change(screen.getByLabelText("Course for Meal"), {
      target: { value: "2" },
    });
  ...
  it("covers existing-order round selection, pending and table-required states", () => {
    ...
    render(<CartSummary {...props} />);
    fireEvent.change(screen.getByLabelText("This round"), {
      target: { value: "2" },
    });
