...
vi.mock("../../../features/branches/hooks/useBranches", () => ({
  useBranches: (arg: any) => h.useBranches(arg),
}));
vi.mock("../../../shared/auth/active-context", () => ({
  persistActiveContext: h.persist,
}));
vi.mock("../../../shared/auth/permissions", () => ({
  userHasPermission: (...args: any[]) => h.permission(...args),
}));
...
beforeEach(() => {
  vi.clearAllMocks();
  h.permission.mockReturnValue(true);
...
describe("BranchSwitcher", () => {
  ...
  it("uses membership branches for branch-scoped users and shows empty state", () => {
    h.useAuthStore.mockReturnValue(
      auth({ memberships: [branchMembership], branchId: "missing" }),
    );
    ...
    fireEvent.click(screen.getByRole("button", { name: /branch/i }));
    expect(
      screen.queryByRole("menuitem", { name: /All Branches/i }),
    ).toBeNull();
    expect(screen.getByRole("menuitem", { name: /Main/i })).toBeTruthy();
    first.unmount();
    ...
    h.useAuthStore.mockReturnValue(
      auth({
        memberships: [{ ...branchMembership, branches: [] }],
        branchId: null,
      }),
    );
  ...
  it("falls back to an empty branch list when tenant and membership branch data are absent", () => {
    h.useBranches.mockReturnValue({ data: undefined });
    h.useAuthStore.mockReturnValue(
      auth({ memberships: [{ ...tenantMembership, branches: undefined }] }),
    );
    render(<BranchSwitcher />);
  ...
  it("supports backdrop/outside close and toggle close", () => {
    ...
    fireEvent.click(trigger);
    fireEvent.click(screen.getByLabelText("Close branch menu"));
    expect(screen.queryByRole("menu")).toBeNull();
    fireEvent.click(trigger);
    fireEvent.click(trigger);
    expect(screen.queryByRole("menu")).toBeNull();
