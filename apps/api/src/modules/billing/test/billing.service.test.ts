...
describe("billing service", () => {
  beforeEach(() => {
    ...
    mocks.findActiveItemsForSeatSplit.mockResolvedValue({
      orderBranchId: "b1",
      items: [],
    });
    mocks.splitOrderByShares.mockResolvedValue(splitOk());
    mocks.replaceSeatShares.mockResolvedValue({
      status: "ok",
      orderBranchId: "b1",
    });
    mocks.findBillsByOrder.mockResolvedValue({
      orderBranchId: "b1",
      bills: [{ id: "bill1" }],
    });
    ...
    mocks.findBillById.mockResolvedValue({
      orderBranchId: "b1",
      bill: { id: "bill1" },
    });
    mocks.buildFractional.mockReturnValue(null);
    mocks.buildSeat.mockReturnValue({
      status: "complete",
      allocations: [{ orderItemIds: ["oi1"] }],
    });
  });
  ...
  it("maps every payment repository outcome", async () => {
    ...
    await expect(
      billingService.createPayment(baseAuth, input),
    ).rejects.toMatchObject({ details: { reason: "ORDER_NOT_FOUND" } });
    mocks.recordPayment.mockResolvedValueOnce({ status: "bill_not_found" });
    await expect(
      billingService.createPayment(baseAuth, input),
    ).rejects.toMatchObject({ details: { reason: "BILL_NOT_FOUND" } });
    mocks.recordPayment.mockResolvedValueOnce({ status: "bill_required" });
    await expect(
      billingService.createPayment(baseAuth, input),
    ).rejects.toMatchObject({ details: { reason: "BILL_REQUIRED" } });
    mocks.recordPayment.mockResolvedValueOnce({
      status: "payment_exceeds_due",
    });
    await expect(
      billingService.createPayment(baseAuth, input),
    ).rejects.toMatchObject({
      details: { reason: "PAYMENT_AMOUNT_EXCEEDS_DUE" },
    });
  });
  ...
  it("creates partial payments and publishes/audits them", async () => {
    ...
    ).resolves.toMatchObject({ paymentState: "PARTIALLY_PAID" });
    expect(mocks.writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({ action: "PAYMENT_CREATED", entityId: "p1" }),
    );
    expect(mocks.publish).toHaveBeenCalledWith(
      expect.objectContaining({ type: "payment.updated" }),
      "t1",
      "b1",
    );
  });
  ...
  it("enforces payment branch access", async () => {
    ...
    ).rejects.toThrow("Billing branch access denied");
  });
  ...
  it("maps split-even outcomes and audits successful splits", async () => {
    ...
    await expect(
      billingService.splitOrder(baseAuth, "o1", 2),
    ).rejects.toMatchObject({ details: { reason: "ORDER_NOT_FOUND" } });
    mocks.splitOrderEvenly.mockResolvedValueOnce({
      status: "already_paid",
      orderBranchId: "b1",
    });
    await expect(
      billingService.splitOrder(baseAuth, "o1", 2),
    ).rejects.toMatchObject({ details: { reason: "BILL_ALREADY_PAID" } });
    mocks.splitOrderEvenly.mockResolvedValueOnce({
      status: "too_many_bills",
      orderBranchId: "b1",
    });
    await expect(
      billingService.splitOrder(baseAuth, "o1", 2),
    ).rejects.toMatchObject({ details: { reason: "TOO_MANY_BILLS" } });
    ...
    expect(mocks.writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({
        action: "BILL_SPLIT",
        metadata: expect.objectContaining({ ways: 2 }),
      }),
    );
  });
  ...
  it("maps split-by-item outcomes and audits success", async () => {
    ...
    mocks.splitOrderByItems.mockResolvedValueOnce({
      status: "order_not_found",
    });
    await expect(
      billingService.splitOrderByItems(baseAuth, "o1", allocations),
    ).rejects.toMatchObject({ details: { reason: "ORDER_NOT_FOUND" } });
    mocks.splitOrderByItems.mockResolvedValueOnce({
      status: "already_paid",
      orderBranchId: "b1",
    ...
    await expect(
      billingService.splitOrderByItems(baseAuth, "o1", allocations),
    ).rejects.toMatchObject({ details: { reason: "EMPTY_BILL" } });
    ...
    expect(mocks.writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({
        metadata: expect.objectContaining({ mode: "ITEM" }),
      }),
    );
  });
  ...
  it("maps fractional seat-plan states and repository outcomes", async () => {
    ...
    await expect(
      billingService.splitOrderBySeat(baseAuth, "o1", "EVEN_SPLIT"),
    ).rejects.toMatchObject({ details: { reason: "ORDER_NOT_FOUND" } });

    mocks.buildFractional.mockReturnValueOnce({ status: "no_seats" });
    await expect(
      billingService.splitOrderBySeat(baseAuth, "o1", "EVEN_SPLIT"),
    ).rejects.toMatchObject({ details: { reason: "NO_SEAT_LABELS" } });

    mocks.buildFractional.mockReturnValueOnce({
      status: "manual_required",
      allocations: [{ label: "S1", orderItemIds: [] }],
      sharedItemIds: ["oi1"],
    });
    await expect(
      billingService.splitOrderBySeat(baseAuth, "o1", "MANUAL"),
    ).resolves.toMatchObject({
      status: "MANUAL_REQUIRED",
      sharedItemIds: ["oi1"],
    });

    mocks.buildFractional.mockReturnValue({
      status: "complete",
      allocations: [
        { label: "S1", itemShares: [{ orderItemId: "oi1", shareRatio: 1 }] },
      ],
    });
    mocks.splitOrderByShares.mockResolvedValueOnce({
      status: "order_not_found",
    });
    await expect(
      billingService.splitOrderBySeat(baseAuth, "o1", "EVEN_SPLIT"),
    ...
    await expect(
      billingService.splitOrderBySeat(baseAuth, "o1", "EVEN_SPLIT"),
    ).rejects.toMatchObject({ details: { reason: "UNKNOWN_ITEM" } });
    mocks.splitOrderByShares.mockResolvedValueOnce(splitOk());
    await expect(
      billingService.splitOrderBySeat(baseAuth, "o1", "EVEN_SPLIT"),
    ).resolves.toMatchObject({ status: "CREATED", bills: expect.any(Array) });
  });
  ...
  it("maps classic seat-plan states and delegates completed plans", async () => {
    ...
    mocks.buildSeat.mockReturnValueOnce({
      status: "manual_required",
      allocations: [{ orderItemIds: [] }],
      sharedItemIds: ["oi1"],
    });
    await expect(
      billingService.splitOrderBySeat(baseAuth, "o1", "MANUAL"),
  ...
  it("validates item seat shares before persistence", async () => {
    await expect(
      billingService.setItemSeatShares(baseAuth, "o1", "oi1", []),
    ).rejects.toMatchObject({ details: { reason: "EMPTY_BILL" } });
    await expect(
      billingService.setItemSeatShares(baseAuth, "o1", "oi1", [
    ...
    for (const shares of [
      ...
      [
        ...
        { seatLabel: "B", shareRatio: 0.4 },
      ],
    ]) {
      await expect(
        billingService.setItemSeatShares(baseAuth, "o1", "oi1", shares),
      ).rejects.toMatchObject({ details: { reason: "INVALID_RATIO" } });
    }
  });

  it("maps seat-share persistence outcomes and audits success", async () => {
    const shares = [{ seatLabel: "A", shareRatio: 1 }];
    mocks.replaceSeatShares.mockResolvedValueOnce({
      status: "order_not_found",
    });
    await expect(
      billingService.setItemSeatShares(baseAuth, "o1", "oi1", shares),
  ...
  it("gets order bills with ownership checks", async () => {
    ...
    await expect(
      billingService.getOrderBills(baseAuth, "o1"),
    ).rejects.toMatchObject({ details: { reason: "ORDER_NOT_FOUND" } });
    mocks.findBillsByOrder.mockResolvedValueOnce({
      orderBranchId: "b1",
      bills: [{ id: "b1" }],
    });
    await expect(billingService.getOrderBills(baseAuth, "o1")).resolves.toEqual(
      [{ id: "b1" }],
    );
  });
  ...
  it("maps refund outcomes and audits successful refunds", async () => {
    ...
    await expect(
      billingService.createRefund(baseAuth, input),
    ).rejects.toMatchObject({ details: { reason: "PAYMENT_NOT_FOUND" } });
    mocks.recordRefund.mockResolvedValueOnce({
    ...
    await expect(billingService.createRefund(baseAuth, input)).resolves.toEqual(
      { id: "r1" },
    );
    expect(mocks.writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({ action: "REFUND_CREATED", entityId: "r1" }),
    );
  });
  ...
  it("gets bills by id and maps missing bills", async () => {
    ...
    await expect(
      billingService.getBill(baseAuth, "bill1"),
    ).rejects.toMatchObject({ details: { reason: "BILL_NOT_FOUND" } });
    mocks.findBillById.mockResolvedValueOnce({
      orderBranchId: "b1",
      bill: { id: "bill1" },
    });
    await expect(billingService.getBill(baseAuth, "bill1")).resolves.toEqual({
      id: "bill1",
    });
  });
});
