...
describe("MenuPicker coverage",()=>{
  ...
  it("covers food-type and search filtered-out categories plus non-dine-in/no-table paths",()=>{
    const props=baseProps();
    props.orderType="TAKEAWAY"; props.tablesEnabled=false; props.filter="VEG" as any; props.categories=[...(props.categories ?? []),{id:"c2",name:"Meat",menuItems:[item({id:"i3",name:"Steak",foodType:"NON_VEG"})]}] as any;
    const {rerender}=render(<MenuPicker {...props}/>);
