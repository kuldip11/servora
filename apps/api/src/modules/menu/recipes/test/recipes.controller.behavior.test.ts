...
describe("recipes controller coverage", () => {
  it("delegates get and set", async () => {
    ...
    await expect(
      recipesController.getItemRecipe(auth, "i1"),
    ).resolves.toMatchObject({ data: [{ id: "r1" }] });
    const ingredients = [
      { inventoryItemId: "inv1", quantity: 1, unit: "GRAM" },
    ] as any;
    await expect(
      recipesController.setItemRecipe(auth, "i1", ingredients),
    ).resolves.toMatchObject({ data: [{ id: "r2" }] });
