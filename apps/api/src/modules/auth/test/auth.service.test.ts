...
describe("auth service edge coverage", () => {
  it("covers signup lookup failure and role-description fallback", async () => {
    ...
    findUserById.mockResolvedValueOnce({
      ...
      globalUserRoles: [
        {
          roleId: "r1",
          role: { name: "OWNER", description: null, rolePermissions: [] },
        },
      ],
    });
    await expect(
  ...
  it("rejects duplicate/missing/inactive login users and exercises unlocked failure", async () => {
    ...
    await expect(
      authService.login({ email: "x", password: "p" } as any),
    ).rejects.toThrow("Invalid credentials");
    ...
    findUsersByEmail.mockResolvedValueOnce([
      { ...user, failedLoginAttempts: 1, lockedUntil: null },
    ]);
    await expect(
      authService.login({ email: "x", password: "wrong" } as any),
    ).rejects.toThrow("Invalid credentials");
  ...
  it("covers membership access early denials and role-id fallbacks", async () => {
    ...
    await expect(
      authService.login({ email: "x", password: "secret" } as any, "web"),
    ).rejects.toThrow("Account does not have access");

    resolveMembership.mockResolvedValueOnce({
      id: "m1",
      tenantId: "t1",
      roles: [],
      branches: [],
  ...
  it("covers logout without a session and session list/revoke behavior", async () => {
    revokeRefreshToken
      ...
      .mockResolvedValueOnce({ id: "rt", userId: "u1", sessionId: null });
    await authService.logout("a");
    await authService.logout("b");
    ...
    await expect(authService.revokeSession("u1", "s0")).rejects.toThrow(
      "Session not found",
    );
    revokeSession.mockResolvedValueOnce({ id: "s1" });
    await expect(authService.revokeSession("u1", "s1")).resolves.toEqual({
      revoked: true,
    });
  });
  ...
  it("covers every invalid refresh-session state", async () => {
    ...
    await expect(authService.refresh("web.x", "web")).rejects.toThrow(
      "Invalid refresh token",
    );
    consumeRefreshToken.mockResolvedValueOnce({
      userId: "u1",
      sessionId: null,
    });
    ...
    for (const session of [
      ...
      { id: "s", revokedAt: null, expiresAt: new Date(Date.now() - 1000) },
    ]) {
      consumeRefreshToken.mockResolvedValueOnce({
        userId: "u1",
        sessionId: "s",
  ...
  it("covers me without membership and all invalid membership states", async () => {
    ...
    findUserById.mockResolvedValue(user);
    await expect(authService.me("u1")).resolves.toEqual({
      user,
      membership: undefined,
    });
    for (const membership of [
      undefined,
      { id: "m", userId: "u2", status: "ACTIVE" },
      { id: "m", userId: "u1", status: "INACTIVE" },
    ]) {
      findMembershipById.mockResolvedValueOnce(membership);
      await expect(authService.me("u1", "m")).rejects.toThrow(
        "Membership access denied",
      );
    }
  });
  ...
  it("filters inaccessible memberships and covers allowed custom membership", async () => {
    const memberships = [
      { membershipId: "m1", tenant: { id: "t1" } },
      { membershipId: "m2", tenant: { id: "t2" } },
      { membershipId: "m3", tenant: { id: "t3" } },
    ];
    ...
    await expect(authService.memberships("u1", "web")).resolves.toEqual([
      memberships[2],
    ]);
  });
  ...
  it("covers profile no-op and missing update", async () => {
    ...
    await expect(
      authService.updateProfile("u1", { firstName: "X" }),
    ).rejects.toThrow();
  });
  ...
  it("covers password user-not-found, same-password and failed-update paths", async () => {
    ...
    const bcrypt = await import("bcryptjs");
    const hash = await bcrypt.default.hash("same", 1);
  ...
  it("covers direct token issuance null user, existing session and role-description fallback", async () => {
    await expect(authService._issueTokens(undefined as any)).rejects.toThrow(
      "User not found",
    );
    saveRefreshToken.mockResolvedValue({});
    touchSession.mockResolvedValue(undefined);
    await expect(
      authService._issueTokens(
        {
          ...user,
          globalUserRoles: [
            {
