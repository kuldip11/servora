...
vi.mock("@/features/staff/services/roles.service", () => ({
  rolesService: { create: mocks.create, archive: mocks.archive },
}));
vi.mock("@/features/staff/services/permissions.service", () => ({
  permissionsService: {
    list: mocks.listPermissions,
    forRole: mocks.forRole,
    setForRole: mocks.setForRole,
  },
}));
...
describe("RoleManager coverage", () => {
  ...
  it("creates and archives custom roles while protecting system roles", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Role name"), {
      target: { value: "Cash Lead" },
    });
    fireEvent.change(screen.getByLabelText("Description"), {
      target: { value: "Cashier lead" },
    });
  ...
  it("loads, toggles and saves permissions", async () => {
    ...
    await waitFor(() =>
      expect(mocks.setForRole).toHaveBeenCalledWith("r2", ["p2"]),
    );
  ...
  it("handles permission/create/archive/save failures and hidden controls", async () => {
    ...
    expect(
      screen.queryByLabelText("Manage permissions for Shift Lead"),
    ).toBeNull();

    rerender(
      <RoleManager roles={[customRole]} canManage canManagePermissions />,
    );
