...
const item: any = {
  ...
  basePrice: "100",
  variants: [
    { id: "v1", name: "Small", price: "100", status: "ACTIVE" },
    { id: "v2", name: "Large", price: "140", status: "ACTIVE" },
    { id: "v3", name: "Sold", price: "160", status: "INACTIVE" },
  ],
  modifierGroupLinks: [
    {
      group: {
...
describe("ItemCustomizerModal coverage", () => {
  ...
  it("covers variants, required/dependent modifiers, quantities, course and confirm", () => {
    ...
    expect(
      (screen.getByRole("button", { name: /Sold/ }) as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    ...
    expect(
      (screen.getByText("Corn").closest("button") as HTMLButtonElement)
        .disabled,
    ).toBe(true);
    const oliveRow = screen.getByText("Olive").closest("div.w-full")!;
    const oliveButtons = oliveRow.querySelectorAll("button");
    fireEvent.click(oliveButtons[2]!);
    fireEvent.click(oliveButtons[2]!);
    fireEvent.click(oliveButtons[1]!);
    const qty = screen.getByText("1", { selector: "span.w-8" });
    const qtyBox = qty.parentElement!;
    fireEvent.click(qtyBox.querySelectorAll("button")[1]!);
    fireEvent.click(qtyBox.querySelectorAll("button")[0]!);
  ...
  it("toggles single option, multiple option and enforces max selection", () => {
    render(
      <ItemCustomizerModal item={item} onConfirm={vi.fn()} onClose={vi.fn()} />,
    );
  ...
  it("shows schema validation errors and restores existing cart defaults", () => {
    ...
    mocks.schema.mockReturnValueOnce({
      success: false,
      error: { issues: [{ message: "Invalid customisation" }] },
    });
    const confirm = vi.fn();
    render(
      <ItemCustomizerModal
    ...
    expect(
      (screen.getByLabelText("Note for Chef") as HTMLInputElement).value,
    ).toBe("old");
    expect(
  ...
  it("covers optional single-group label and option quantity clamping", () => {
    ...
    expect(screen.getByText("Optional · choose 1")).toBeTruthy();
    fireEvent.click(screen.getByText("Green").closest("button")!);
    fireEvent.click(screen.getByText("Green").closest("button")!);
  });

  it("covers no-variant/no-modifier defaults", () => {
    const plain = {
      id: "p",
      name: "Plain",
      basePrice: "20",
