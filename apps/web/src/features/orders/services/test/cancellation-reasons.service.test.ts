...
vi.mock("@pos/api-client", () => ({
  createOrdersApi: () => ({
    ...
    createCancellationReason: h.create,
    updateCancellationReason: h.update,
  }),
}));
...
describe("cancellationReasonsService coverage", () => {
  ...
  it("covers active/inactive listing and mutations", async () => {
    h.list.mockResolvedValue([1]);
    h.listAll.mockResolvedValue([2]);
    h.create.mockResolvedValue({ id: "r1" });
    h.update.mockResolvedValue({ id: "r1", label: "Changed" });
    ...
    expect(await cancellationReasonsService.create("Sold out")).toEqual({
      id: "r1",
    });
    expect(
    ...
    ).toEqual({ id: "r1", label: "Changed" });
    expect(h.create).toHaveBeenCalledWith("Sold out");
    expect(h.update).toHaveBeenCalledWith("r1", {
      label: "Changed",
      isActive: false,
    });
  });
