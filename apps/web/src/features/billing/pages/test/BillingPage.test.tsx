...
vi.mock("@pos/ui", () => ({
  ...
  Pagination: ({ onPageChange, onPageSizeChange }: any) => (
    <div>
      <button onClick={() => onPageChange(2)}>page2</button>
      <button onClick={() => onPageSizeChange(50)}>size50</button>
    </div>
  ),
  ...
  Table: ({ columns, data, emptyTitle }: any) => (
    <div>
      {data.length ? (
        data.map((row: any) => (
          <div key={row.id}>
...
describe("BillingPage coverage", () => {
  beforeEach(() => {
    ...
    mocks.orders.mockReturnValue({
      data: { items: [order], pagination: { total: 1 } },
      isLoading: false,
    });
  });
  ...
  it("covers bill selection, item summaries, validation, and non-cash references", () => {
        ...
        itemAssignments: [
          {
            ...
            orderItem: {
              ...
              quantity: 1,
              taxMode: "INCLUSIVE",
              order: { id: order.id, table: { name: "4" } },
            },
          },
          {
            orderItemId: "i3",
    ...
    fireEvent.change(screen.getByLabelText("Reference / Transaction ID"), {
      target: { value: "txn-1" },
    });
    fireEvent.change(screen.getByLabelText("Amount"), {
      target: { value: "0" },
  ...
  it("covers printing persisted bills and checkbox selection", () => {
    ...
    fireEvent.click(screen.getByText("Print selected"));
    expect(mocks.printBills).toHaveBeenCalledWith(order, [
      expect.objectContaining({ id: "due" }),
    ]);
    fireEvent.click(dueCheckbox);
    expect(
      (screen.getByText("Print selected") as HTMLButtonElement).disabled,
    ).toBe(true);
  });
  ...
  it("covers item split allocation and combo-group remapping", () => {
    ...
    mocks.orders.mockReturnValue({
      data: { items: [splitOrderData], pagination: { total: 1 } },
      isLoading: false,
    });
    render(<BillingPage />);
    fireEvent.click(screen.getByText("Split"));
    ...
    const itemSelects = screen
      .getAllByRole("combobox")
      .filter(
        (element) => !(element as HTMLSelectElement).getAttribute("aria-label"),
      );
    ...
    expect(mocks.splitItems).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([
        expect.objectContaining({
          label: "Bill 3",
          orderItemIds: expect.arrayContaining(["i2", "i3"]),
        }),
      ]),
    );
  });
  ...
  it("covers seat split success and manual-required conversion", () => {
    ...
    mocks.splitSeat.mockReturnValue({
      ...
      allocations: [{ orderItemIds: ["i1"] }, { orderItemIds: ["i2"] }],
      sharedItemIds: ["i3"],
    });
    render(<BillingPage />);
    fireEvent.click(screen.getByText("Split"));
