...
describe("persisted active context", () => {
  ...
  it("persists and clears context explicitly", () => {
    ...
    expect(
      JSON.parse(localStorage.getItem("servora.active-context.v1")!),
    ).toEqual({ membershipId: "m1", franchiseId: "f1", branchId: "b1" });
    clearPersistedContext();
    expect(localStorage.getItem("servora.active-context.v1")).toBeNull();
  });

  it("returns false when no memberships exist", async () => {
    expect(await restoreActiveContext([])).toBe(false);
  ...
  it("clears active identifiers and rethrows when user refresh fails", async () => {
    ...
    h.me.mockRejectedValueOnce(error);
