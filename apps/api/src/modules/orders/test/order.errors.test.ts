...
it("creates item and table-operation errors", () => {
  ...
  expect(errors.orderItemNotFound("i1").toJSON()).toMatchObject({
    code: "NOT_FOUND",
  });
  expect(errors.orderItemCannotBeVoided("void").details).toMatchObject({
    reason: "ORDER_ITEM_NOT_VOIDABLE",
  });
  expect(errors.orderItemCannotBeComped("comp").details).toMatchObject({
    reason: "ORDER_ITEM_NOT_COMPABLE",
  });
  expect(errors.orderItemCannotBeRefired("refire").details).toMatchObject({
    reason: "ORDER_ITEM_NOT_REFIREABLE",
  });
  expect(errors.orderCannotTransferTable("transfer").details).toMatchObject({
    reason: "ORDER_TABLE_TRANSFER_FAILED",
  });
  expect(errors.orderCannotMerge("merge").details).toMatchObject({
    reason: "ORDER_MERGE_FAILED",
  });
});
