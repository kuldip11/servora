...
describe("billing errors", () => {
  ...
  it("maps payment and bill rules to stable domain errors", () => {
    ...
    expect(paymentExceedsDueAmount().details).toMatchObject({
      reason: "PAYMENT_AMOUNT_EXCEEDS_DUE",
    });
    expect(billSelectionRequired().details).toMatchObject({
      reason: "BILL_REQUIRED",
    });
    expect(splitBillNotAllowed("BILL_ALREADY_PAID").message).toBe(
      "A paid order cannot be split",
    );
    expect(splitBillNotAllowed("TOO_MANY_BILLS").message).toBe(
