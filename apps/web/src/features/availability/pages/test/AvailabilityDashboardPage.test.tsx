...
describe("AvailabilityDashboardPage", () => {
  ...
  it("shows API errors and supports filtering, sorting and scoped reloads", async () => {
    ...
    expect(await screen.findByText("Availability dashboard unavailable")).toBeTruthy();
    expect(screen.getByText("dashboard down")).toBeTruthy();
    failed.unmount();

    api.get.mockResolvedValue({ data: { data: { rows: [
      { entityType: "ITEM", entityId: "2", menuItemId: "2", name: "Zulu", status: "OUT_OF_STOCK", reason: "No stock", cause: "RECIPE_DRIVEN", branchId: "b1", branchName: null, channel: "STAFF", fulfillmentType: "DINE_IN" },
      { entityType: "MODIFIER_OPTION", entityId: "1", menuItemId: "1", name: "Alpha", status: "HIDDEN", reason: "Disabled", cause: "MANUAL", branchId: "b1", branchName: "Main", channel: "CUSTOMER_QR", fulfillmentType: "TAKEAWAY" },
      { entityType: "MODIFIER_OPTION", entityId: "1", menuItemId: "1", name: "Alpha", status: "HIDDEN", reason: "Disabled", cause: "MANUAL", branchId: "b1", branchName: "Main", channel: "CUSTOMER_QR", fulfillmentType: "TAKEAWAY" }
    ] } } });
    render(<AvailabilityDashboardPage />);
    expect(await screen.findByText("Alpha")).toBeTruthy();
    expect(screen.getByText(/Current branch/)).toBeTruthy();
    fireEvent.change(screen.getByLabelText("Search availability exceptions"), { target: { value: "main" } });
    ...
    await waitFor(() => expect(api.get).toHaveBeenLastCalledWith("/menu/availability/dashboard", expect.objectContaining({ params: expect.objectContaining({ channel: "STAFF", fulfillmentType: "DELIVERY", cause: "RECIPE_DRIVEN" }) })));
