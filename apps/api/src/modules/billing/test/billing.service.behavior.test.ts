...
describe("billingService coverage", () => {
  beforeEach(() => {
    ...
    mocks.splitOrderEvenly.mockResolvedValue({
      status: "ok",
      orderBranchId: "b1",
      bills: [{ id: "b1" }, { id: "b2" }],
    });
    mocks.splitOrderByItems.mockResolvedValue({
      status: "ok",
      orderBranchId: "b1",
      bills: [{ id: "b1" }],
    });
    ...
    mocks.splitOrderByShares.mockResolvedValue({
      status: "ok",
      orderBranchId: "b1",
      bills: [{ id: "b1" }],
    });
    ...
    mocks.seatPlan.mockReturnValue({
      status: "complete",
      allocations: [{ label: "Seat 1", orderItemIds: ["i1"] }],
    });
  });
  ...
  it("creates partial and fully-paid payments, audits, and publishes order/table releases", async () => {
    const partial = await billingService.createPayment(auth, {
      orderId: "o1",
      method: "CARD",
      amount: 10,
      reference: "ref",
    ...
    const paid = await billingService.createPayment(auth, {
      orderId: "o1",
      method: "CASH",
      amount: 10,
    });
  ...
  it("maps even split outcomes and audits success", async () => {
    for (const [status, reason] of [
      ...
      await expect(
        billingService.splitOrder(auth, "o1", 2),
    ...
    await expect(billingService.splitOrder(auth, "o1", 2)).resolves.toEqual([
      { id: "b1" },
    ]);
    expect(mocks.writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({
        action: "BILL_SPLIT",
        metadata: { ways: 2, billIds: ["b1"] },
      }),
    );
  });
  ...
  it("maps item split outcomes and audits success", async () => {
    ...
    await expect(
      billingService.splitOrderByItems(auth, "o1", []),
    ).rejects.toMatchObject({ details: { reason: "ORDER_NOT_FOUND" } });
    mocks.splitOrderByItems.mockResolvedValueOnce({
      status: "already_paid",
      orderBranchId: "b1",
    ...
    await expect(
      billingService.splitOrderByItems(auth, "o1", allocations),
  ...
  it("handles fractional seat plans: no seats, manual, repository failures, and created bills", async () => {
    ...
    await expect(
      billingService.splitOrderBySeat(auth, "missing", "EVEN_SPLIT"),
    ).rejects.toMatchObject({ details: { reason: "ORDER_NOT_FOUND" } });

    mocks.fractionalPlan.mockReturnValueOnce({ status: "no_seats" });
    await expect(
      billingService.splitOrderBySeat(auth, "o1", "EVEN_SPLIT"),
    ).rejects.toMatchObject({ details: { reason: "NO_SEAT_LABELS" } });

    mocks.fractionalPlan.mockReturnValueOnce({
      status: "manual_required",
      allocations: [{ label: "S1", orderItemIds: [] }],
      sharedItemIds: ["i1"],
    });
    await expect(
      billingService.splitOrderBySeat(auth, "o1", "MANUAL"),
    ).resolves.toEqual({
      status: "MANUAL_REQUIRED",
      allocations: [{ label: "S1", orderItemIds: [] }],
      sharedItemIds: ["i1"],
    ...
    await expect(
      billingService.splitOrderBySeat(auth, "o1", "EVEN_SPLIT"),
    ).resolves.toEqual({ status: "CREATED", bills: [{ id: "b1" }] });
  });
  ...
  it("handles classic seat plans: no seats, manual, and complete delegating to item split", async () => {
    ...
    mocks.seatPlan.mockReturnValueOnce({
      status: "manual_required",
      allocations: [],
      sharedItemIds: ["i1"],
    });
    ...
    mocks.seatPlan.mockReturnValueOnce({
      status: "complete",
      allocations: [{ label: "S1", orderItemIds: ["i1"] }],
    });
    mocks.splitOrderByItems.mockResolvedValueOnce({
      status: "ok",
      orderBranchId: "b1",
  ...
  it("validates item seat shares and maps repository outcomes", async () => {
    ...
    for (const shares of invalidCases) {
      await expect(
        billingService.setItemSeatShares(auth, "o1", "i1", shares as any),
      ).rejects.toThrow();
    }
    ...
    for (const result of [
      ...
      await expect(
        billingService.setItemSeatShares(auth, "o1", "i1", [
    ...
    await expect(
      billingService.setItemSeatShares(auth, "o1", "i1", shares),
    ).resolves.toBe(shares);
    expect(mocks.writeAudit).toHaveBeenCalledWith(
      expect.objectContaining({ action: "ORDER_ITEM_SEAT_SHARES_UPDATED" }),
    );
  });
  ...
  it("gets order bills and maps missing order", async () => {
    ...
    await expect(
      billingService.getOrderBills(auth, "o1"),
    ).rejects.toMatchObject({ details: { reason: "ORDER_NOT_FOUND" } });
    mocks.findBillsByOrder.mockResolvedValueOnce({
      orderBranchId: "b1",
      bills: [{ id: "b1" }],
    });
    await expect(billingService.getOrderBills(auth, "o1")).resolves.toEqual([
      { id: "b1" },
    ]);
  });
