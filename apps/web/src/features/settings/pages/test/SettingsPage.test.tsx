...
vi.mock("@/features/settings/components/PricingSettingsCard", () => ({
  PricingSettingsCard: ({ tenantId }: any) => <div>pricing-{tenantId}</div>,
}));
vi.mock("@/features/settings/components/KitchenOperationsSettingsCard", () => ({
  KitchenOperationsSettingsCard: ({ tenantId }: any) => (
    <div>kitchen-{tenantId}</div>
  ),
}));
...
describe("SettingsPage coverage", () => {
  beforeEach(() => {
    ...
    mocks.auth.mockReturnValue({
      user: {
        ...
        branchId: "branch-123456789",
        roles: [{ permissions }],
      },
      franchiseId: "fr1",
    });
    mocks.reasons.mockReturnValue({
      data: [
        { id: "r1", label: "Mistake", isActive: true },
        { id: "r2", label: "Sold out", isActive: false },
      ],
    });
  ...
  it("renders permission-enabled settings and updates cancellation reasons", async () => {
    ...
    await waitFor(() =>
      expect(mocks.update).toHaveBeenCalledWith("r1", { isActive: false }),
    );
    fireEvent.click(screen.getByText("Enable"));
    await waitFor(() =>
      expect(mocks.update).toHaveBeenCalledWith("r2", { isActive: true }),
    );
    ...
    fireEvent.click(add);
    await waitFor(() =>
      expect(mocks.create).toHaveBeenCalledWith("Guest changed mind"),
    );
  ...
  it("handles mutation failure and restricted settings", async () => {
    ...
    mocks.auth.mockReturnValue({
      user: { tenantId: "tenant", branchId: null, roles: [] },
      franchiseId: null,
    });
