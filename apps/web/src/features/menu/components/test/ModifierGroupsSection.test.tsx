...
const groups = [
  ...
  },
  {
    ...
    minSelections: 0,
    maxSelections: null,
    dependsOnOptionId: "11111111-1111-4111-8111-111111111111",
    options: [
      {
...
describe("ModifierGroupsSection coverage", () => {
  ...
  it("creates a dependent multi-option group", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Min required"), {
      target: { value: "1" },
    });
    fireEvent.change(screen.getByLabelText("Show only after option"), {
      target: { value: "11111111-1111-4111-8111-111111111111" },
    });
    fireEvent.change(screen.getByLabelText("Option 1 name"), {
      target: { value: " Mayo " },
    });
    fireEvent.change(screen.getByLabelText("Option 1 additional price"), {
      target: { value: "5" },
    });
    fireEvent.change(screen.getByLabelText("Option 1 max quantity"), {
      target: { value: "2" },
    });
    fireEvent.click(screen.getByText("+ Add option"));
    fireEvent.change(screen.getByLabelText("Option 2 name"), {
      target: { value: " Ketchup " },
    });
    fireEvent.change(screen.getByLabelText("Option 2 max quantity"), {
      target: { value: "1" },
    });
    ...
    expect(arg.payload.options).toEqual([
      expect.objectContaining({
        ...
        maxQuantity: 2,
        isDefault: true,
        replacesDefaultComponent: "Aioli",
      }),
      expect.objectContaining({
        name: "Ketchup",
        additionalPrice: 0,
  ...
  it("edits, removes, cancels and confirms deletion", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Edit Modifier Group" }),
    ).toBeTruthy();
    expect(
      (screen.getByLabelText("Group name") as HTMLInputElement).value,
    ).toBe("Sides");
    expect(
      (screen.getByLabelText("Max allowed") as HTMLInputElement).value,
    ).toBe("1");
    expect(
      (
        screen.getByLabelText("Show only after option") as HTMLSelectElement
      ).querySelector('option[value="11111111-1111-4111-8111-111111111111"]'),
    ).toBeNull();
    fireEvent.change(screen.getByLabelText("Selection"), {
      target: { value: "MULTIPLE" },
    });
    fireEvent.change(screen.getByLabelText("Max allowed"), {
      target: { value: "3" },
    });
    fireEvent.change(screen.getByLabelText("Show only after option"), {
    ...
    expect(mocks.save.mock.calls[0]![0]).toEqual(
      expect.objectContaining({
        ...
        payload: expect.objectContaining({
          maxSelections: 3,
          dependsOnOptionId: "33333333-3333-4333-8333-333333333333",
        }),
      }),
    );

    const currentIcons = screen
      .getAllByRole("button")
      .filter((button) => !button.textContent?.trim());
    fireEvent.click(currentIcons[2]!);
    fireEvent.click(screen.getByRole("button", { name: "Cancel" }));
    const afterCancel = screen
      .getAllByRole("button")
      .filter((button) => !button.textContent?.trim());
