...
describe("SubRecipeManager coverage", () => {
  ...
  it("covers listing deletion and full inventory ingredient create/edit/remove", async () => {
    ...
    fireEvent.click(screen.getByLabelText("Delete Sauce"));
    await waitFor(() => expect(h.remove).toHaveBeenCalledWith("s1"));
    fireEvent.click(screen.getByRole("button", { name: /New sub-recipe/ }));
    fireEvent.change(screen.getByLabelText("Name"), {
      target: { value: " Component " },
    });
    fireEvent.change(screen.getByLabelText("Batch yield"), {
      target: { value: "3" },
    });
    fireEvent.change(screen.getByLabelText("Yield unit"), {
      target: { value: "LITERS" },
    });
    fireEvent.change(screen.getByLabelText("Yield % (optional)"), {
      target: { value: "75" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Add ingredient" }));
    let selects = screen.getAllByRole("combobox");
    fireEvent.change(selects[2]!, { target: { value: "i2" } });
    fireEvent.change(
      screen.getByLabelText("Sub-recipe ingredient quantity 1"),
      { target: { value: "2.5" } },
    );
    fireEvent.change(selects[3]!, { target: { value: "ML" } });
    ...
    expect(h.create.mock.calls[0][0]).toMatchObject({
      ...
      yieldUnit: "LITERS",
      yieldPercent: 75,
      ingredients: [{ inventoryItemId: "i2", quantity: 2.5, unit: "ML" }],
    });
    expect(h.success).toHaveBeenCalledWith("Sub-recipe created");
  });
  it("covers sub-recipe source selection and ingredient removal/validation", async () => {
    ...
    fireEvent.change(selects[2]!, { target: { value: "s2" } });
    fireEvent.click(
      screen
        .getAllByRole("button")
        .find(
          (b) =>
            b.querySelector("svg") &&
            !/New sub-recipe/.test(b.textContent || ""),
        )!,
    );
  });
  ...
  it("reports missing ingredient sources and mutation errors", async () => {
    inventory = [];
    subs = [];
    h.create.mockRejectedValueOnce(new Error("create"));
    h.remove.mockRejectedValueOnce(new Error("delete"));
    render(<SubRecipeManager />);
    fireEvent.click(screen.getByRole("button", { name: /New sub-recipe/ }));
    fireEvent.click(screen.getByRole("button", { name: "Add ingredient" }));
    expect(h.error).toHaveBeenCalledWith(
      undefined,
      expect.stringContaining("Add raw inventory"),
    );
    ...
    const second = render(<SubRecipeManager />);
    fireEvent.click(second.getByLabelText("Delete Sauce"));
    await waitFor(() =>
      expect(h.error).toHaveBeenCalledWith(
        expect.any(Error),
        "Could not delete sub-recipe",
      ),
    );
  });
});
