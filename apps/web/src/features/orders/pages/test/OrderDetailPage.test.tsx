...
vi.mock("@tanstack/react-router", () => ({
  useParams: () => ({ orderId: "order-12345678" }),
...
vi.mock("@/features/orders/hooks/useUpdateOrderStatus", () => ({
  useUpdateOrderStatus: () => mocks.updateStatus,
}));
vi.mock("@/features/orders/hooks/useUpdateTicketStatus", () => ({
  useUpdateTicketStatus: () => mocks.updateTicket,
}));
vi.mock("@/features/orders/hooks/useRefireOrderItem", () => ({
  useRefireOrderItem: () => mocks.refire,
}));
vi.mock("@/features/orders/hooks/useSetOrderItemSeatShares", () => ({
  useSetOrderItemSeatShares: () => mocks.seats,
}));
vi.mock("@/features/orders/hooks/useVoidOrderItem", () => ({
  useVoidOrderItem: () => mocks.voidItem,
}));
...
vi.mock("@/store/auth", () => ({
  useAuthStore: (selector: any) =>
    selector({ user: { roles: [{ name: "OWNER" }] } }),
}));
vi.mock("@/features/orders/utils/round-actions", () => ({
...
vi.mock("@/shared/lib/api-client", () => ({
  ...
  toApiClientError: (e: unknown) => ({
    code:
      e instanceof Error && e.message === "Manager approval required"
...
vi.mock("@/features/orders/components/AddItemsModal", () => ({
  AddItemsModal: ({ onClose }: { onClose: () => void }) => (
    <div data-testid="add-items">
      <button onClick={onClose}>close add</button>
    </div>
  ),
}));
vi.mock("@/features/orders/components/OrderExplainDialog", () => ({
  OrderExplainDialog: ({ onClose }: { onClose: () => void }) => (
    <div data-testid="explain">
      <button onClick={onClose}>close explain</button>
    </div>
  ),
}));
vi.mock("@/features/orders/components/ReasonDialog", () => ({
  ReasonDialog: ({ onSubmit, onClose }: any) => (
    <div data-testid="reason">
      <button
        onClick={() =>
          onSubmit({ reason: "Mistake", cancellationReasonId: "r1" })
        }
      >
        submit reason
      </button>
...
vi.mock("@/features/orders/components/ManagerApprovalDialog", () => ({
  ManagerApprovalDialog: ({ onApproved, onClose }: any) => (
    <div data-testid="approval">
      <button onClick={() => onApproved("token")}>approve</button>
...
vi.mock("@pos/ui", () => ({
  ...
  PageHeader: ({ title, description, actions, eyebrow }: any) => (
    <header>
      {eyebrow}
      <h1>{title}</h1>
      <p>{description}</p>
...
const item = (
  ...
  subtotal: "50",
  itemStatus: status,
  modifiers: [{ modifierId: `mod-${id}`, name: "Cheese", quantity: 2 }],
  ...extra,
});
const ticket = (id: string, status: string, items: any[]) => ({
  id,
  ticketNumber: 1,
  status,
  notes: `note-${id}`,
...
const fullOrder = (status = "OPEN") => ({
  ...
  items: [
    item("active"),
    item("void", "VOIDED", { voidedReason: "Mistake" }),
    item("refire", "REFIRED", { refireReason: "Cold", compedAt: "x" }),
    item("replacement", "ACTIVE", { refiresOrderItemId: "refire" }),
    item("comp", "COMPED", { compedReason: "Service" }),
  ],
  kitchenTickets: [
    ticket("held", "HELD", [item("active")]),
    ticket("fired", "FIRED", [
      item("fired", "ACTIVE", {
        variantName: "Large",
        chefNotes: "hot",
    ...
    ticket("prep", "PREPARING", [item("prep")]),
    ticket("ready", "READY", [item("ready")]),
  ],
  statusHistory: [
    {
      id: "h1",
      changedAt: "2026-09-04T12:05:00Z",
      newStatus: "OPEN",
      reason: "Created",
      cancellationReason: { label: "None" },
    },
  ],
});
...
describe("OrderDetailPage coverage", () => {
  ...
  it("covers cancel, comp approval, and paid-to-closed transitions", () => {
    mocks.compItem.mutate.mockImplementationOnce(
  ...
  it("covers seat-share editing, add/remove validation, and refire billing toggle", () => {
    ...
    orderWithShares.kitchenTickets = [
      ticket("held", "HELD", [
        item("active", "ACTIVE", {
          seatShares: [
    ...
    expect((screen.getByText("Save split") as HTMLButtonElement).disabled).toBe(
      true,
    );
    ...
    expect(mocks.seats.mutate).toHaveBeenCalledWith(
      expect.objectContaining({
        shares: expect.arrayContaining([
          expect.objectContaining({ seatLabel: "VIP", shareRatio: 0.4 }),
        ]),
      }),
      expect.any(Object),
    );
  ...
  it("covers course headings, unknown statuses, no-ticket orders, and optional pricing fields", () => {
    mocks.order.mockReturnValue({
      data: {
        ...
        discountAmount: "0",
        items: [
          item("tax-in", "ACTIVE", { taxMode: "INCLUSIVE" }),
          item("tax-ex", "ACTIVE", {
            taxMode: "EXCLUSIVE",
            modifiers: [{ modifierId: "m", name: "Sauce", quantity: 1 }],
          }),
        ],
        kitchenTickets: [
          {
            ...ticket("course", "SERVED", []),
  ...
  it("covers mutation success callbacks and modal close handlers", () => {
    ...
    mocks.seats.mutate.mockImplementationOnce(
      (_vars: unknown, options: { onSuccess?: () => void }) =>
        options.onSuccess?.(),
    );
    mocks.refire.mutate.mockImplementationOnce(
      (_vars: unknown, options: { onSuccess?: () => void }) =>
        options.onSuccess?.(),
    );
    mocks.updateStatus.mutate.mockImplementationOnce(
      (_vars: unknown, options?: { onSuccess?: () => void }) =>
        options?.onSuccess?.(),
    );
    render(<OrderDetailPage />);
  ...
  it("exposes bill and close transitions for terminal ticket/order states", () => {
    mocks.order.mockReturnValue({
      data: {
        ...fullOrder("OPEN"),
        kitchenTickets: [ticket("served", "SERVED", [item("x")])],
      },
      isLoading: false,
    });
