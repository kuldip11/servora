...
describe("ItemCustomizerModal coverage",()=>{
 ...
 it("covers variants, required/dependent modifiers, quantities, course and confirm",()=>{
  ...
  const oliveRow=screen.getByText("Olive").closest("div.w-full")!; const oliveButtons=oliveRow.querySelectorAll("button"); fireEvent.click(oliveButtons[2]!); fireEvent.click(oliveButtons[2]!); fireEvent.click(oliveButtons[1]!);
 ...
 it("covers optional single-group label and option quantity clamping",()=>{
  const optionalSingle={...item,modifierGroupLinks:[{group:{id:"g4",name:"Optional Sauce",selectionType:"SINGLE",minSelections:0,maxSelections:null,options:[{id:"o6",name:"Green",additionalPrice:"1",isAvailable:true,maxQuantity:1}]}}]} as any;
  render(<ItemCustomizerModal item={optionalSingle} onConfirm={vi.fn()} onClose={vi.fn()}/>);
