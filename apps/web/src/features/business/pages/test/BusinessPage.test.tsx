...
describe("BusinessPage coverage", () => {
  ...
  it("covers franchise and branch form controls, capability toggles, cancellation and declined archive confirmations", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Brand One" }));
    fireEvent.click(screen.getByRole("button", { name: /Edit/ }));
    const cuisine = screen.getByLabelText("Cuisine types");
    fireEvent.change(cuisine, { target: { value: "Indian, Cafe, " } });
    const franchiseChecks = screen.getAllByRole("checkbox");
    fireEvent.click(franchiseChecks[0]!);
    fireEvent.click(franchiseChecks[franchiseChecks.length - 1]!);
    vi.mocked(globalThis.confirm).mockReturnValueOnce(false);
    fireEvent.click(screen.getByRole("button", { name: "Archive" }));
    expect(mocks.archiveFranchise).not.toHaveBeenCalled();
    ...
    vi.mocked(globalThis.confirm).mockReturnValueOnce(false);
    fireEvent.click(screen.getByRole("button", { name: "Archive" }));
    expect(mocks.archiveOrganization).not.toHaveBeenCalled();
  });

  it("reports save and archive failures for organization, franchise, and branch mutations", async () => {
    render(<BusinessPage />);
