...
describe("menu membership service coverage", () => {
  it("lists memberships and menu items with read permission", async () => {
    ...
    await expect(membershipService.listForItem(auth(["menu:read"]), "item1")).resolves.toEqual([{ id: "m1" }]);
    await expect(membershipService.listItems(auth(["menu:read"]), "menu1")).resolves.toEqual([{ id: "m2" }]);
  });
  ...
  it("rejects missing assignment resources", async () => {
    for (const resources of [
      ...
      await expect(membershipService.assign(auth(["menu:update"]), "item1", { menuId: "menu1", categoryId: "cat1" })).rejects.toThrow("Menu, item, or category not found");
    }
  });

  it("allows unscoped categories and defaults sort order to zero", async () => {
    findResources.mockResolvedValue({ menu: { id: "menu1" }, item: { id: "item1", branchId: "b1" }, category: { id: "cat1", branchId: null } });
    upsert.mockResolvedValue({ id: "member1" });
    await membershipService.assign(auth(["menu:update"]), "item1", { menuId: "menu1", categoryId: "cat1" });
    expect(upsert).toHaveBeenCalledWith({ menuId: "menu1", menuItemId: "item1", categoryId: "cat1", sortOrder: 0 });
    expect(record).toHaveBeenCalledWith(expect.anything(), "MENU_MEMBERSHIP", "member1", "UPDATED", expect.objectContaining({ sortOrder: 0 }));
  });
  ...
  it("removes memberships and records the change", async () => {
    ...
    expect(record).toHaveBeenCalledWith(expect.anything(), "MENU_MEMBERSHIP", "item1", "DELETED", { menuId: "menu1", menuItemId: "item1" });
  });
});
