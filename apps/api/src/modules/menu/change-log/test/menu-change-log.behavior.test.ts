...
vi.mock("@/db", () => {
  ...
  return {
    db: {
      insert: m.insert,
      select: m.select,
      query: { menuChangeEvents: { findMany: m.findMany } },
    },
  };
});
...
describe("menu change log comprehensive coverage", () => {
  it("builds diffs for unchanged, added, removed, and changed values", () => {
    ...
    expect(buildDiff({ removed: true }, null)).toEqual({
      removed: { old: true, new: null },
    });
  });
  ...
  it("records one event and many events including the empty fast path", async () => {
    ...
    await expect(
      menuChangeLog.recordMany({ tenantId: "t1", userId: "u1" }, []),
    ).resolves.toEqual([]);
  ...
  it("lists with and without every optional filter and caps limits", async () => {
    ...
    expect(m.findMany).toHaveBeenLastCalledWith(
  ...
  it("returns latest unique event ids per item and handles empty ids", async () => {
    await expect(
      menuChangeLog.latestForItems("t1", [], new Date()),
    ...
    const out = await menuChangeLog.latestForItems(
      "t1",
      ["i1", "i1", "i2"],
      new Date("2026-01-01"),
    );
