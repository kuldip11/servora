...
describe("templates route coverage", () => {
  it("executes all handlers", async () => {
    for (const fn of Object.values(controller))
      (fn as any).mockResolvedValue({ ok: true });
    ...
    expect(
      (
        await req("/api/menu/templates/from-category/c1", {
          method: "POST",
    ...
    expect(
      (
        await req("/api/menu/templates/t1/apply", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ branchId: "b1", categoryName: "New" }),
        })
      ).status,
    ).toBe(201);
    expect(
      (await req("/api/menu/templates/t1", { method: "DELETE" })).status,
    ).toBe(200);
