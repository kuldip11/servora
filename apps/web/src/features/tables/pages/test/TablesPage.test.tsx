...
vi.mock("@/features/tables/hooks/useTablesRealtimeSync", () => ({
  useTablesRealtimeSync: vi.fn(),
}));
vi.mock("@/features/tables/hooks/useCreateTable", () => ({
  useCreateTable: () => ({ mutate: mocks.add, isPending: false }),
}));
vi.mock("@/features/tables/hooks/useUpdateTable", () => ({
  useUpdateTable: () => ({ mutate: mocks.update, isPending: false }),
}));
vi.mock("@/features/tables/hooks/useUpdateTableStatus", () => ({
  useUpdateTableStatus: () => ({ mutate: mocks.status }),
}));
vi.mock("@/features/tables/hooks/useDeleteTable", () => ({
  useDeleteTable: () => ({ mutate: mocks.del }),
}));
...
vi.mock("@/features/tables/components/TableFormModal", () => ({
  TableFormModal: ({ open, mode, onSubmit, onClose }: any) =>
    open ? (
      <div role="dialog">
        <h2>{mode}-table</h2>
        <button
        ...
        {mode === "add" ? (
          <button
            onClick={() =>
              onSubmit({ name: "T0", capacity: "2", section: "", branchId: "" })
            }
          >
            submit-add-no-branch
          </button>
        ) : null}
...
vi.mock("@pos/ui", () => ({
  ...
  Modal: ({ open, title, children, onClose, footer }: any) =>
    open ? (
      <div role="dialog">
        <h2>{title}</h2>
...
describe("TablesPage coverage", () => {
  ...
  it("filters tables and handles add/edit/status/delete and QR flows", async () => {
    render(<TablesPage />);
    expect(screen.getByText("3 of 3 tables")).toBeTruthy();
    fireEvent.change(screen.getByLabelText("Search tables"), {
      target: { value: "A2" },
    });
    expect(screen.getByText("1 of 3 tables")).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: /Available 1/ }));
    fireEvent.change(screen.getByLabelText("Filter tables by section"), {
      target: { value: "Hall" },
    });
    ...
    expect(mocks.add).toHaveBeenCalledWith(
      expect.objectContaining({ name: "T9", capacity: 4, section: "Hall" }),
      expect.any(Object),
    );
    ...
    fireEvent.change(screen.getByLabelText("status-AVAILABLE"), {
      target: { value: "CLEANING" },
    });
    expect(mocks.status).toHaveBeenCalledWith({ id: "t2", status: "CLEANING" });
    ...
    expect(
      screen.getByRole("heading", { name: "A2 — Customer QR" }),
    ).toBeTruthy();
    fireEvent.click(screen.getByRole("button", { name: "Regenerate" }));
    expect(mocks.regen).toHaveBeenCalledWith("t2", expect.any(Object));
  ...
  it("loads takeaway QR and transfers/merges occupied tables", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Central — Takeaway QR" }),
    ).toBeTruthy();
    ...
    fireEvent.change(selects.at(-1)!, { target: { value: "t2" } });
    fireEvent.change(screen.getByLabelText("Reason (optional)"), {
      target: { value: "guest move" },
    });
    fireEvent.click(
    ...
    const transferOptions = mocks.transfer.mock.calls.at(-1)?.[1];
    act(() => transferOptions.onSuccess());
    fireEvent.click(screen.getByRole("button", { name: "Merge" }));
    expect(screen.getByRole("heading", { name: "Merge A1" })).toBeTruthy();
  ...
  it("covers aggregate grouping, loading and empty filtered states", () => {
    ...
    const { rerender } = render(<TablesPage />);
  ...
  it("covers close, error, permission, aggregate mutation, disabled QR and pagination paths", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /Add Table/ }));
    fireEvent.click(
      screen.getByRole("button", { name: "submit-add-no-branch" }),
    );
    expect(mocks.add).not.toHaveBeenCalledWith(
      expect.objectContaining({ name: "T0" }),
      expect.anything(),
    );
    fireEvent.click(screen.getByRole("button", { name: "close-add" }));
    fireEvent.click(screen.getAllByLabelText("Remove table")[0]!);
    expect(mocks.del).toHaveBeenCalled();
    fireEvent.change(screen.getAllByLabelText("status-AVAILABLE")[0]!, {
      target: { value: "CLEANING" },
    });
    ...
    mocks.has.mockImplementation(
      (permission: string) =>
        permission !== "orders:update" && permission !== "tables:create",
    );
    useAuthStore.setState({ branchId: "b1" });
    render(<TablesPage />);
  ...
  it("covers mutation success callbacks, missing transfer order, merge errors and large-table observer", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByRole("heading", { name: "add-table" })).toBeNull(),
    );
    fireEvent.click(screen.getAllByLabelText("Edit table")[1]!);
    fireEvent.click(screen.getByRole("button", { name: "submit-edit" }));
    ...
    fireEvent.click(screen.getAllByLabelText("Show table QR code")[1]!);
    fireEvent.click(screen.getByRole("button", { name: "Regenerate" }));
    const regenOptions = mocks.regen.mock.calls.at(-1)?.[1];
    act(() => regenOptions.onSuccess({ ...t2, publicQrToken: "new-q" }));
