...
describe("PromotionsSection coverage", () => {
  beforeEach(() => {
    ...
    mocks.stats = {
      p1: { uses: 3, discountAmount: "12.50" },
      p2: { uses: 0, discountAmount: "0" },
    };
    mocks.categories = [
      {
        id: "c1",
  ...
  it("creates ordinary scoped promotion with limits and uppercase coupon", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Max uses total"), {
      target: { value: "100" },
    });
    fireEvent.change(screen.getByLabelText("Max uses / customer"), {
      target: { value: "2" },
    });
    fireEvent.click(screen.getByLabelText(/Stackable with loyalty/));
  ...
  it("creates BOGO item/category rewards and validates required targets", async () => {
    render(<PromotionsSection />);
    fireEvent.change(screen.getByLabelText("Name"), {
      target: { value: "BOGO" },
    });
    ...
    fireEvent.change(screen.getByLabelText("Buy item"), {
      target: { value: "i1" },
    });
    fireEvent.change(screen.getByLabelText("Reward target"), {
      target: { value: "ITEM" },
    });
    fireEvent.change(screen.getByLabelText("Reward item"), {
      target: { value: "i2" },
    });
    fireEvent.change(screen.getByLabelText("Buy quantity"), {
      target: { value: "3" },
    });
    fireEvent.change(screen.getByLabelText("Reward quantity"), {
      target: { value: "2" },
    });
    fireEvent.change(screen.getByLabelText("Reward discount %"), {
      target: { value: "50" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Create promotion" }));
    await waitFor(() =>
    ...
    fireEvent.change(screen.getByLabelText("Buy category"), {
      target: { value: "c1" },
    });
    fireEvent.change(screen.getByLabelText("Reward target"), {
      target: { value: "CATEGORY" },
  ...
  it("edits, cancels, toggles active state, deletes and renders stats placeholders", async () => {
    render(<PromotionsSection />);
    expect(screen.getAllByText("Loading stats…").length).toBeGreaterThan(0);
    fireEvent.click(screen.getAllByRole("button", { name: "Edit" })[0]!);
    expect((screen.getByLabelText("Name") as HTMLInputElement).value).toBe(
      "Lunch 10",
    );
    expect(screen.getByRole("button", { name: "Save promotion" })).toBeTruthy();
    fireEvent.change(screen.getByLabelText("Type"), {
    ...
    await waitFor(() =>
      expect(mocks.update).toHaveBeenCalledWith("p1", { isActive: false }),
    );
    fireEvent.click(screen.getByRole("button", { name: "Enable" }));
    await waitFor(() =>
      expect(mocks.update).toHaveBeenCalledWith("p2", { isActive: true }),
    );
