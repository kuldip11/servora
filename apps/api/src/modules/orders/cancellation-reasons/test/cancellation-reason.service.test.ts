...
it("covers validation and optional update patch branches", async () => {
  await expect(cancellationReasonService.create(auth(), "   ")).rejects.toThrow(
    "label is required",
  );
  mocks.findById.mockResolvedValue({ id: "r1" });
  await expect(
    cancellationReasonService.update(auth(), "r1", { label: "   " }),
  ...
  await expect(
    cancellationReasonService.assertUsable("t1", "r1"),
  ).resolves.toBeUndefined();
});
