...
describe("promotionService complete coverage", () => {
  ...
  it("covers list and stats", async () => {
    ...
    await expect(promotionService.stats(auth, "p1")).resolves.toEqual({
      uses: 2,
      discountAmount: "5.00",
    });
  });
  ...
  it("rejects fixed, BOGO, scope, and date validation failures", async () => {
    ...
    ).rejects.toThrow(/exactly one trigger/i);
    await expect(
      promotionService.create(auth, {
        name: "x",
        ruleType: "BOGO",
    ...
    ).rejects.toThrow(/reward discount/i);
    await expect(
      promotionService.create(auth, {
        name: "x",
        ruleType: "BOGO",
    ...
    await expect(
      promotionService.create(auth, {
        ...
        ruleType: "PERCENTAGE",
        scope: "ORDER",
        scopeCategoryId: "cat-a",
        value: 10,
      }),
    ).rejects.toThrow(/cannot target/i);
    await expect(
      promotionService.create(auth, {
        name: "x",
        ruleType: "PERCENTAGE",
    ...
    ).rejects.toThrow(/requires exactly one category/i);
    await expect(
      promotionService.create(auth, {
        name: "x",
        ruleType: "PERCENTAGE",
    ...
    ).rejects.toThrow(/requires exactly one item/i);
    await expect(
      promotionService.create(auth, {
        name: "x",
        ruleType: "PERCENTAGE",
  ...
  it("covers update fallback branches for persisted optional fields", async () => {
    ...
    await expect(
      promotionService.update(auth, "pc", { name: "Keep fallbacks" }),
    ).resolves.toMatchObject({ name: "Keep fallbacks" });
    ...
    await expect(
      promotionService.update(auth, "pb", { name: "Bogo kept" }),
    ).resolves.toMatchObject({ id: "pb" });
    expect(update).toHaveBeenLastCalledWith(
      "tenant",
      "pb",
      expect.objectContaining({
  ...
  it("covers explicit update branches for scope and BOGO target fields", async () => {
    ...
    await expect(
      promotionService.update(auth, "px", {
        scope: "ITEM",
        scopeCategoryId: null,
        scopeMenuItemId: "item-a",
      }),
    ).resolves.toMatchObject({ scope: "ITEM" });
    ...
    update.mockResolvedValueOnce(existingBogo);
    await expect(
      promotionService.update(auth, "pbx", {
        triggerMenuItemId: null,
        triggerCategoryId: "cat-a",
        rewardMenuItemId: "item-a",
        rewardCategoryId: null,
        rewardDiscountPercent: 75,
        triggerQuantity: 3,
        rewardQuantity: 2,
      }),
  ...
  it("covers remaining update fallback statements", async () => {
    ...
    findById.mockResolvedValueOnce(existingItem);
    update.mockResolvedValueOnce(existingItem);
    await expect(
      promotionService.update(auth, "pi", { name: "Item kept" }),
    ...
    update.mockResolvedValueOnce(existingBogo);
    await expect(
      promotionService.update(auth, "pbf", { name: "Bogo fallback kept" }),
    ).resolves.toMatchObject({ id: "pbf" });
  });
  ...
  it("covers update missing, successful merge, failed update, remove and missing remove", async () => {
    ...
    await expect(
      promotionService.update(auth, "missing", { name: "x" }),
    ...
    await expect(
      promotionService.update(auth, "p1", {
        name: "New",
        value: 20,
        couponCode: null,
        startDate: null,
    ...
    await expect(
      promotionService.update(auth, "p1", { value: 15 }),
    ).rejects.toThrow(/not found/i);

    findById.mockResolvedValueOnce(undefined);
    await expect(
      promotionService.remove(auth, "missing"),
