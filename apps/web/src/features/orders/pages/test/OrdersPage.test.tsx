...
vi.mock("@pos/ui", () => ({
  ...
  Table: ({ columns, data, onSortChange, emptyAction }: any) => (
    <div data-testid="table">
      <button
        onClick={() => onSortChange?.({ columnId: "time", direction: "asc" })}
      >
        sort time
      </button>
      <button
        onClick={() => onSortChange?.({ columnId: "total", direction: "desc" })}
      >
        sort total
      </button>
      <button
        onClick={() => onSortChange?.({ columnId: "other", direction: "asc" })}
      >
        sort other
      </button>
      {data.length
  ...
  FilterBar: ({
    children,
    onClearAll,
  }: React.PropsWithChildren<{ onClearAll?: () => void }>) => (
    <div>
      {children}
...
describe("OrdersPage coverage", () => {
      ...
      data: {
        items: [
          ...
          order({
            ...
            table: null,
            status: "UNKNOWN",
            kitchenTickets: [{ id: "k2", status: "SERVED" }],
          }),
          order({
            id: "multi-12345678",
            kitchenTickets: [
  ...
  it("renders rows and drives filters, sorting, paging, and create modal", () => {
    ...
    fireEvent.change(screen.getByLabelText("Filter orders by type"), {
    ...
    expect(mocks.useOrdersPage).toHaveBeenLastCalledWith(
      expect.objectContaining({ sortBy: "total", sortDirection: "desc" }),
    );
