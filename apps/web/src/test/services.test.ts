...
const h = vi.hoisted(() => ({
  org: { list: vi.fn(), tenants: vi.fn(), create: vi.fn(), update: vi.fn(), archive: vi.fn() },
  branches: { list: vi.fn(), create: vi.fn(), update: vi.fn(), deactivate: vi.fn() },
  audit: { list: vi.fn(), menuHistory: vi.fn() },
  orders: { listCancellationReasons: vi.fn(), listAllCancellationReasons: vi.fn(), createCancellationReason: vi.fn(), updateCancellationReason: vi.fn() },
...
describe("web service coverage", () => {
  ...
  it("covers business CRUD and normalization", async () => {
    ...
    expect(h.branches.create).toHaveBeenCalledWith(expect.objectContaining({ address: "Road", onlineEnabled: true, isActive: true, phone: "" }));
  ...
  it("covers audit and cancellation-reason services", async () => {
    await auditService.list(); await auditService.list(10, "cursor"); await auditService.menuHistory(); await auditService.menuHistory({ entityType: "MENU", limit: 5 });
    ...
    await cancellationReasonsService.list(); await cancellationReasonsService.list(false); await cancellationReasonsService.listAll(); await cancellationReasonsService.create("Mistake"); await cancellationReasonsService.update("r1", { label: "Changed", isActive: false });
  ...
  it("covers billing optional fields and split helpers", async () => {
    ...
    await billingService.getOrderBills("o1"); await billingService.splitOrder("o1", 2); await billingService.splitOrderByItems("o1", []); await billingService.splitOrderBySeat("o1", "EVEN_SPLIT");
  ...
  it("covers analytics service branches", async () => {
    await analyticsService.dashboard(); await analyticsService.costMargin(); await analyticsService.costMargin("c1");
