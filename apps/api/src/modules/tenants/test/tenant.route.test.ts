...
vi.mock("../tenant.controller", () => ({ tenantController: mocks }));
import { tenantsRouter } from "../tenant.route";
const route = (m: string, p: string) =>
  (tenantsRouter as any).routes.find(
    (r: any) => r.method === m && r.path === p,
  );
describe("tenant routes coverage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("executes every route", async () => {
    const auth = { tenantId: "t1" };
    expect(await route("GET", "/").handler({ auth })).toBe("list");
    const set: any = {};
    expect(
      await route("POST", "/").handler({
        auth,
        body: { name: "X", organizationId: "o1" },
        set,
      }),
    ).toBe("create");
    ...
    expect(
      await route("DELETE", "/:id").handler({ auth, params: { id: "t1" } }),
    ).toBe("archive");
    expect((tenantsRouter as any).routes).toHaveLength(4);
  });
});
