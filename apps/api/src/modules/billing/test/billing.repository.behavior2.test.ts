...
vi.mock("@/db", () => {
  const tx = {
    query: {
      ...
      orderItems: {
        findMany: mocks.txItemFindMany,
        findFirst: mocks.txItemFindFirst,
      },
  ...
  return {
    db: {
      ...
      query: {
        ...
        bills: {
          findMany: mocks.dbBillFindMany,
          findFirst: mocks.dbBillFindFirst,
        },
...
describe("billingRepository additional coverage", () => {
  it("covers splitOrderEvenly outcomes and successful replacement", async () => {
    ...
    ).resolves.toMatchObject({ status: "order_not_found" });

    mocks.txOrderFindFirst.mockResolvedValueOnce({
      ...baseOrder,
      branchId: "b2",
    });
    await expect(
  ...
  it("covers splitOrderByItems validation/combo failures and success", async () => {
    ...
    await expect(
      billingRepository.splitOrderByItems({
        orderId: "o1",
        allocations: [{ orderItemIds: ["i1", "i2"] }],
        tenantId: "t1",
        branchId: null,
      }),
    ).resolves.toMatchObject({ status: "already_paid" });
  ...
  it("covers seat-share replacement outcomes and write paths", async () => {
    ...
    await expect(
      billingRepository.replaceSeatShares({
        ...
        branchId: null,
        shares: [
          { seatLabel: " Seat 1 ", shareRatio: 0.5 },
          { seatLabel: "Seat 2", shareRatio: 0.5 },
        ],
      }),
    ).resolves.toMatchObject({ status: "ok" });
  });
  ...
  it("finds order bills and active seat-split items with branch scoping", async () => {
    ...
    mocks.dbOrderFindFirst.mockResolvedValueOnce({
      ...baseOrder,
      branchId: "b2",
    });
    await expect(
    ...
    ).resolves.toEqual({ bills: [{ id: "b1" }], orderBranchId: "b1" });
    await expect(
      billingRepository.findActiveItemsForSeatSplit({
        orderId: "o1",
        tenantId: "t1",
    ...
    ).resolves.toEqual({ orderBranchId: "b1", items: activeItems });
  });
  ...
  it("covers refund success, update race, and tenant-wide lookup", async () => {
    ...
    mocks.txUpdateWhere.mockReturnValueOnce({
      returning: vi.fn().mockResolvedValue([]),
    });
    await expect(
    ...
    ).resolves.toMatchObject({ status: "not_refundable" });
    ...
    mocks.txInsert.mockReturnValueOnce(
      insertReturning([{ id: "r1", amount: "5.00" }]),
    );
    await expect(
      billingRepository.recordRefund({
        paymentId: "p1",
    ...
    ).resolves.toMatchObject({ status: "ok", refund: { id: "r1" } });
  });
  ...
  it("covers recordPayment merged-order, bill-selection, and due validation paths", async () => {
    ...
    mocks.txSelect.mockReturnValueOnce({
      from: vi.fn(() => ({
        where: vi.fn().mockResolvedValue([{ total: "8.00" }]),
      })),
    });
    await expect(billingRepository.recordPayment(input)).resolves.toEqual({
  ...
  it("assigns active items when recordPayment creates the first bill", async () => {
    ...
    mocks.txInsert
      .mockReturnValueOnce(
        insertReturning([{ id: "new-bill", totalAmount: "34.00" }]),
      )
      .mockReturnValueOnce(insertPlain())
      .mockReturnValueOnce(
        insertReturning([{ id: "p1", amount: "5.00", status: "SUCCESS" }]),
      );
    mocks.txSelect
      .mockReturnValueOnce({
        from: vi.fn(() => ({
      ...
      .mockReturnValueOnce({
        from: vi.fn(() => ({
          where: vi.fn(() => ({
            groupBy: vi
              .fn()
              .mockResolvedValue([{ billId: "new-bill", total: "5.00" }]),
          })),
        })),
      });
  ...
  it("settles a merged paid order, records status history, and releases tables", async () => {
    ...
    mocks.txInsert
      .mockReturnValueOnce(
        insertReturning([{ id: "p1", amount: "10.00", status: "SUCCESS" }]),
      )
      .mockReturnValueOnce(insertPlain());
    ...
    const orderUpdate = {
      set: vi.fn(() => ({
        where: vi.fn(() => ({
          returning: vi.fn().mockResolvedValue(paidOrders),
        })),
      })),
    };
    const tableUpdate = {
      set: vi.fn(() => ({
        where: vi.fn(() => ({
          returning: vi
            .fn()
            .mockResolvedValue([{ id: "table1", status: "AVAILABLE" }]),
        })),
      })),
    };
    mocks.txUpdate
