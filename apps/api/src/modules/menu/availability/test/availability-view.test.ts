...
describe("availability view helpers", () => {
  it("resolves modifier availability with and without manual overrides", () => {
    expect(effectiveModifierAvailability({ computedAvailability: true })).toBe(
      true,
    );
    expect(
      effectiveModifierAvailability({
    ...
    expect(
      withEffectiveModifierAvailability({
        ...
        manualOverrideAvailability: true,
        id: "o1",
      }),
    ).toEqual({
      computedAvailability: false,
  ...
  it("resolves item availability from status, override, and manual stock", () => {
    ...
    expect(
      effectiveMenuItemAvailability({ status: "ACTIVE", manualStockCount: 0 }),
    ).toBe(false);
    expect(
      effectiveMenuItemAvailability({ status: "ACTIVE", manualStockCount: 2 }),
    ).toBe(true);
    expect(
      effectiveMenuItemAvailability({
        status: "HIDDEN",
    ...
    expect(
      withEffectiveMenuItemAvailability({ status: "ACTIVE", id: "i1" }),
    ).toEqual({ status: "ACTIVE", id: "i1", isAvailable: true });
  });
});
