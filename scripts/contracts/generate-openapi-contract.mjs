...
const input =
  ...
  process.env.SERVORA_OPENAPI_SOURCE ??
  "http://127.0.0.1:5175/swagger/json";
...
const loadSpec = async () => {
  if (/^https?:\/\//.test(input)) {
    ...
    if (!response.ok)
      throw new Error(
        `OpenAPI fetch failed (${response.status}) from ${input}`,
      );
...
const lines = [
  ...
  ...operations.map(
    ({ method, path }) =>
      `  { method: ${JSON.stringify(method)}, path: ${JSON.stringify(path)} },`,
  ),
  "] as const;",
  "",
...
if (checkOnly) {
  ...
  if (current !== generated) {
    console.error(
      "Generated OpenAPI contract is stale. Run `bun run contracts:generate` and commit the result.",
    );
    process.exit(1);
  }
