...
vi.mock("@/db", () => {
  ...
  m.transaction.mockImplementation(async (fn: Function) =>
    fn({ update: m.txUpdate }),
  );
  return {
    db: {
      query: {
        branches: { findMany: m.branchFindMany, findFirst: m.branchFindFirst },
...
describe("branch repository comprehensive coverage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    m.insertReturning.mockResolvedValue([{ id: "b1" }]);
    m.updateReturning.mockResolvedValue([{ id: "b1" }]);
    m.txUpdateReturning.mockResolvedValue([{ id: "b1" }]);
  });
  it("covers list scopes/finds/count", async () => {
    await branchRepository.findMany("t1", "b1");
    await branchRepository.findMany("t1", null, ["b1"]);
    await branchRepository.findMany("t1", null, []);
    await branchRepository.findByCode("t1", "X");
    await branchRepository.findById("t1", "b1");
    await expect(branchRepository.countActive("t1")).resolves.toBe(2);
  });
  it("creates with defaults and explicit capabilities", async () => {
  ...
  it("regenerates QR and deactivates sessions only when branch updated", async () => {
    await expect(
      branchRepository.regenerateTakeawayQr("t1", "b1"),
    ).resolves.toEqual({ id: "b1" });
    expect(m.txUpdate).toHaveBeenCalledTimes(2);
    m.txUpdateReturning.mockResolvedValueOnce([]);
    await expect(
      branchRepository.regenerateTakeawayQr("t1", "missing"),
    ).resolves.toBeUndefined();
  });
  it("updates and checks open order booleans", async () => {
    await expect(
      branchRepository.update("t1", "b1", { name: "N" } as any),
    ).resolves.toEqual({ id: "b1" });
    m.updateReturning.mockResolvedValueOnce([]);
    await expect(
      branchRepository.update("t1", "b1", {}),
    ).resolves.toBeUndefined();
    ...
    await expect(
      branchRepository.hasOpenOrdersOfType("t1", "b1", "DINE_IN"),
    ).resolves.toBe(true);
    m.orderFindFirst.mockResolvedValueOnce(undefined);
    await expect(
      branchRepository.hasOpenOrdersOfType("t1", "b1", "TAKEAWAY"),
    ).resolves.toBe(false);
  });
});
