...
const { findCustomerGroup } = vi.hoisted(() => ({
  findCustomerGroup: vi.fn(),
}));
...
vi.mock("@/modules/customer-groups/customer-group.repository", () => ({
  customerGroupRepository: { findById: findCustomerGroup },
}));
...
describe("price-rule service complete validation coverage", () => {
  it("covers list organization and tenant scopes", async () => {
    ...
    await expect(priceRuleService.list(auth, "item1")).resolves.toEqual([
      { id: "tenant" },
    ]);
    const orgAuth = {
      ...auth,
    ...
    await expect(
      priceRuleService.list(orgAuth, undefined, "org-1", "SKU"),
    ).resolves.toEqual([{ id: "org" }]);
    organizationIdForTenant.mockResolvedValueOnce("org-2");
    await expect(
      priceRuleService.list(orgAuth, undefined, "org-1"),
    ).rejects.toThrow(/outside the active tenant organization/i);
  });

  it("rejects invalid values and item scope combinations", async () => {
    await expect(
      priceRuleService.create(auth, { menuItemId: "item1" } as never),
    ).rejects.toThrow(/exactly one/i);
    await expect(
      priceRuleService.create(auth, {
        menuItemId: "item1",
    ...
    await expect(
      priceRuleService.create(auth, { price: 10 } as never),
    ).rejects.toThrow(/require menuItemId/i);
    ...
    findByIdItem.mockResolvedValueOnce({
      id: "item1",
      branchId: null,
      variants: [{ id: "v1" }],
    });
    ...
    ).rejects.toThrow(/Customer group/i);
  });
  ...
  it("validates organization and per-cover scopes", async () => {
    ...
    organizationIdForTenant.mockResolvedValue("org-1");
    await expect(
      priceRuleService.create(orgAuth, {
    ...
    ).rejects.toThrow(/require menuItemSku/i);
    await expect(
      priceRuleService.create(orgAuth, {
        organizationId: "org-1",
        menuItemSku: "SKU",
    ...
    ).rejects.toThrow(/cannot target tenant-local/i);
    await expect(
      priceRuleService.create(auth, {
        isPerCover: true,
        menuItemId: "item1",
    ...
    ).rejects.toThrow(/cannot target a menu item/i);
    await expect(
      priceRuleService.create(auth, {
        isPerCover: true,
        variantId: "v1",
    ...
    ).rejects.toThrow(/cannot target a variant/i);
    await expect(
      priceRuleService.create(auth, {
        isPerCover: true,
        percentOff: 10,
      } as never),
    ).rejects.toThrow(/absolute price/i);
    await expect(
      priceRuleService.create(auth, {
        menuItemId: "item1",
        coverTier: "ADULT",
    ...
    ).rejects.toThrow(/only valid on a per-cover/i);
  });

  it("persists conversions and organization rules", async () => {
    await priceRuleService.create(auth, {
      menuItemId: "item1",
      price: 12.5,
      taxRate: 5,
    ...
    organizationIdForTenant.mockResolvedValue("org-1");
    listOrganization.mockResolvedValue([]);
    await expect(
      priceRuleService.create(orgAuth, {
        organizationId: "org-1",
        menuItemSku: " SKU ",
  ...
  it("covers update and happy-hour edge branches", async () => {
    ...
    await expect(
      priceRuleService.update(auth, "missing", { price: 10 } as never),
    ...
    await expect(
      priceRuleService.update(auth, "r1", { price: 11 } as never),
    ).rejects.toThrow(/not found/i);
    ...
    ).rejects.toThrow(/no menu items/i);
    ...
    ).rejects.toThrow(/Menu not found/i);
  ...
  it("returns silently when removing a missing rule", async () => {
    findById.mockResolvedValueOnce(undefined);
    await expect(
      priceRuleService.remove(auth, "missing"),
    ).resolves.toBeUndefined();
    expect(remove).not.toHaveBeenCalled();
  });
});
