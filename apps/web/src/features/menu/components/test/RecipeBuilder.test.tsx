...
describe("RecipeBuilder coverage", () => {
  ...
  it("loads recipe, edits scopes/sources and saves valid ingredients", async () => {
    ...
    fireEvent.change(selects[0]!, { target: { value: "sub-recipe" } });
    ...
    const scope = all.find((x) =>
      Array.from(x.querySelectorAll("option")).some(
        (o) => o.value === "variant",
      ),
    )!;
    fireEvent.change(scope, { target: { value: "variant" } });
  ...
  it("adds inventory/sub-recipe rows, modifier scope and removes rows", () => {
    ...
    fireEvent.change(scope, { target: { value: "modifier" } });
    ...
    fireEvent.click(screen.getByText(/Add ingredient/));
    expect(screen.getByDisplayValue("Sauce")).toBeTruthy();
  });
  ...
  it("hydrates variant/modifier scopes and covers source, unit, variant, and modifier selectors", () => {
    ...
    render(<RecipeBuilder item={item} />);
    const selects = screen.getAllByRole("combobox");
    const inventorySelect = selects.find((x) =>
      Array.from(x.querySelectorAll("option")).some((o) => o.value === "inv2"),
    )!;
    ...
    const unitSelect = screen
      ...
      .find((x) =>
        Array.from(x.querySelectorAll("option")).some(
          (o) => o.value === "KILOGRAMS" || o.value === "GRAMS",
        ),
      )!;
    ...
    const modifierSelect = screen
      .getAllByRole("combobox")
      .find((x) =>
        Array.from(x.querySelectorAll("option")).some((o) => o.value === "o1"),
      )!;
