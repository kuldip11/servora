...
vi.mock("@/features/staff/services/staff.service", () => ({
  staffService: { update: mocks.update },
}));
vi.mock("@/features/staff/query-keys", () => ({
  staffKeys: { list: () => ["staff"] },
}));
...
vi.mock("@/features/staff/components/forms/EditStaffForm", () => ({
  EditStaffForm: ({ onSubmit, onCancel }: any) => (
    <div>
      <button
        onClick={() =>
          onSubmit({
...
vi.mock("@/features/staff/components/roles/RoleManager", () => ({
  RoleManager: ({ canManage, canManagePermissions }: any) => (
    <div>
      roles-manager-{String(canManage)}-{String(canManagePermissions)}
    </div>
  ),
}));
...
describe("StaffPage coverage", () => {
  ...
  it("renders staff actions, filters and pagination", async () => {
    render(<StaffPage />);
    expect(screen.getByText("52 team members")).toBeTruthy();
    expect(screen.getByText("Ada Lovelace")).toBeTruthy();
    expect(screen.getByText("—")).toBeTruthy();
    fireEvent.click(screen.getByLabelText("Deactivate"));
    expect(mocks.status).toHaveBeenCalledWith({ id: "s1", status: "INACTIVE" });
    fireEvent.click(screen.getByLabelText("Activate"));
    expect(mocks.status).toHaveBeenCalledWith({ id: "s2", status: "ACTIVE" });
    fireEvent.click(screen.getAllByLabelText("Remove staff member")[0]!);
    expect(mocks.del).toHaveBeenCalledWith("s1");
    fireEvent.change(screen.getByLabelText("Search staff"), {
      target: { value: "Ada" },
    });
    fireEvent.change(screen.getByLabelText("Filter staff by status"), {
      target: { value: "ACTIVE" },
    });
  ...
  it("adds and edits staff and handles update success/error", async () => {
    ...
    await waitFor(() =>
      expect(mocks.update).toHaveBeenCalledWith(
        "s1",
        expect.objectContaining({ firstName: "Edit" }),
      ),
    );
    expect(mocks.success).toHaveBeenCalledWith("Staff member updated");
  ...
  it("covers roles tab, empty team and restricted permissions", () => {
    ...
    expect(screen.getByText("roles-manager-true-false")).toBeTruthy();
