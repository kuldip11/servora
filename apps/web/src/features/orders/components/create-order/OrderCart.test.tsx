...
const props = (): React.ComponentProps<typeof OrderCart> => ({
  items: [] as any[],
  notes: "",
  total: 0,
  pending: false,
...
describe("OrderCart coverage", () => {
  it("covers empty cart, notes, validation, disabled submit and submit callback", () => {
    const p = props();
    p.validationError = "Choose a table";
    render(<OrderCart {...p} />);
    ...
    fireEvent.change(screen.getByPlaceholderText("Order notes..."), {
  ...
  it("covers populated cart details, modifiers, course, edit, qty and removal", () => {
    ...
    const first = makeItem({
      ...
      variantName: "Large",
      modifiers: [
        { optionId: "o1", name: "Cheese", quantity: 2 },
        { optionId: "o2", name: "Sauce", quantity: 1 },
      ],
      courseNumber: 3,
      chefNotes: "Extra hot",
    });
