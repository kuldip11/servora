...
const mocks=vi.hoisted(()=>({mutate:vi.fn(),schema:vi.fn(),course:true,categories:[] as any[],omitOptionQuantity:false}));
...
vi.mock("@/features/orders/services/orders.service",()=>({toCartItemPayload:(x:any)=>({menuItemId:x.menuItemId,quantity:x.quantity,selectedOptions:(x.modifiers??[]).map((m:any)=>mocks.omitOptionQuantity?{optionId:m.optionId}:{optionId:m.optionId,quantity:m.quantity}),chefNotes:x.chefNotes,seatLabel:x.seatLabel,variantId:x.variantId})}));
...
describe("AddItemsModal coverage",()=>{
 ...
 it("covers equal variant pricing, empty filtered categories, sibling increment mapping and default option quantity",async()=>{
  ...
  const customized=payload.items.find((x:any)=>x.menuItemId==="i2");
  expect(customized.selectedOptions[0].quantity).toBe(1);
 });
