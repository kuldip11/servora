  ...
  const alerts = useMemo<OperationalAlert[]>(() => {
    ...
    if (dashboard.lowStockAlerts > 0) {
      next.push({
        ...
        description:
          "Inventory has reached or fallen below configured minimum stock levels.",
        severity: dashboard.lowStockAlerts >= 5 ? "critical" : "warning",
        meta: "Source: authoritative inventory levels",
        action: (
          <Link
            to="/inventory"
            className="inline-flex items-center justify-center rounded-md border border-border bg-surface px-3 py-2 text-sm font-medium text-text-primary transition hover:bg-surface-secondary focus:outline-none focus:ring-2 focus:ring-primary"
          >
            Review inventory
          </Link>
        ),
    ...
    if (dashboard.cancelledOrdersToday > 0) {
      next.push({
        ...
        description:
          "Review cancellation patterns and operational causes before they repeat.",
    ...
    for (const [branchId, rows] of groupedAvailability) {
      ...
      next.push({
        ...
        severity: rows.some((row) => row.status === "OUT_OF_STOCK")
          ? "warning"
          : "info",
    ...
    if (next.length === 0) {
      next.push({
        ...
        description:
          "Orders, inventory thresholds and menu availability are currently within configured operating rules.",
        severity: "success",
      ...
      {snapshot.error ? (
        <Card className="border-danger/30 bg-danger-surface">
          ...
          <p className="mt-1 text-sm text-text-secondary">
            Some source systems could not be queried. Retry before acting on
            this view.
          </p>
        </Card>
      ) : null}
      ...
      {snapshot.data ? (
        <>
          <Grid columns={{ base: 1, sm: 2, lg: 4 }} gap="md">
            <StatCard
              title="Active orders"
              value={snapshot.data.dashboard.activeOrders}
              icon={Activity}
              color="blue"
            />
            <StatCard
              title="Low stock"
              value={snapshot.data.dashboard.lowStockAlerts}
              icon={Package}
              color="amber"
            />
            <StatCard
              title="Availability exceptions"
              value={snapshot.data.availability.length}
              icon={AlertTriangle}
              color="red"
            />
            <StatCard
              ...
              value={
                snapshot.data.branches.filter((branch) => branch.isActive)
                  .length
              }
              icon={GitBranch}
              color="emerald"
          ...
          <div className="mt-6 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-text-secondary">
                Prioritized from authoritative Servora data, not synthetic demo
                alerts.
              </p>
            </div>
            <Badge
