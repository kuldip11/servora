...
const baseProps = (): React.ComponentProps<typeof MenuPicker> => ({
  orderType: "DINE_IN",
  tableId: "",
  tablesEnabled: true,
  tables: [
    { id: "t1", name: "T1", status: "AVAILABLE" },
    { id: "t2", name: "T2", status: "OCCUPIED" },
  ],
  categories: [
    {
      id: "c1",
      ...
      menuItems: [
        item(),
        item({ id: "i2", name: "Salad", basePrice: 80, foodType: "VEG" }),
      ],
    },
  ] as any,
  filter: "ALL" as any,
...
describe("MenuPicker coverage", () => {
  it("covers order/table selectors, search, filtering, item selection and fixed/variant prices", () => {
    ...
    props.categories = [
      {
        ...
        menuItems: [
          item({
            variants: [{ price: 120 }, { price: 150 }],
            modifierGroupLinks: [{ id: "g1" }],
          }),
          item({
            id: "i2",
            name: "Tea",
    ...
    fireEvent.change(screen.getByLabelText("Table (required)"), {
      target: { value: "t1" },
    });
  ...
  it("covers food-type and search filtered-out categories plus non-dine-in/no-table paths", () => {
    ...
    props.tablesEnabled = false;
    props.filter = "VEG" as any;
    props.categories = [
      ...(props.categories ?? []),
      {
        id: "c2",
        name: "Meat",
        menuItems: [item({ id: "i3", name: "Steak", foodType: "NON_VEG" })],
      },
    ] as any;
    const { rerender } = render(<MenuPicker {...props} />);
    ...
    expect(
      screen.getByText("No menu items available for this order."),
    ).toBeTruthy();
  });
});
